...
$mtsql=mysqli_query($conn,"SELECT `name`, `username`, `empid` FROM `nsauth` WHERE `station`='$dept' ORDER BY `name`");
while($mtfetch=mysqli_fetch_array($mtsql)){
  if($mtfetch['username']==$userunique){$opsel="selected";}else{$opsel="";}
echo "
            <option value='".$mtfetch['empid']."' $opsel>".mb_strtoupper($mtfetch['name'])."</option>
";
}
