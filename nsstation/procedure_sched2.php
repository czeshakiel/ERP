...
if($deptx=="OR"){
$pf = $conn->query("select * from orpf where caseno='$caseno'");
while($pfd = $pf->fetch_assoc()){$doc[] = $pfd['doctor']."||".$pfd['specialization'];}
$doc = json_encode($doc);
$type = $_POST['type'];
}
$batchno = "RF".date("YmdHis");
$conn->query("insert into `ORSCHEDULE`(`patientidno`, `caseno`, `dateofoperation`, `timeofoperation`, `typeofoperation`, `room`, `usages`, `status`, `username`, `branch`, `pf`, `type`, `remarks`) 
values ('$patientidno','$caseno','$date','$time','$pro','$deptx','$protype','RESERVED','$user','$branch','$batchno','$type', '$remarks')");

$conn->query("update orpf set batchno='$batchno' where caseno='$caseno'");

echo"<script>window.location='?proceduresched&caseno=$caseno';</script>";
}
?>
...
<option value="<?php echo $viewdept ?>"><?php echo $viewdept ?></option>
...
<tr>
<td>PROCEDURE:<br>
<select name="pro" id="pro" class="select2-single" style="width: 435px;"></select>
</td>
</tr>
<tr><td>REMARKS<br><textarea name="remarks" rows="4" cols="50"></textarea></td></tr>
<tr>
...
<option value="Schedule">Schedule</optioon>
<option value="Oncall">Oncall</optioon>
<option value="Stat">Stat</optioon>
</select>
</td>
</tr>

<tr id='stat1'>
<td>Date :<br><input type="date" name="date" value="<?php echo date('Y-m-d'); ?>" style="width: 100%; font-size: 15px;"></td>
</tr>
...
<option value="MINOR">MINOR OPERATION</option>
...
<?php 
$sqx = $conn->query("select d.name, o.specialization, o.id from orpf o, docfile d where o.doctor=d.code and o.caseno='$caseno' and batchno='' order by o.id asc");
...
$.get("loadpro.php", {str:str, doctor:doctor, services:services, caseno:caseno, str2:str2}, function (data) {$('#myDiv2').load(' #myDiv2');});
}
