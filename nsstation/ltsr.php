...
for($q=1;$q<=$xval;$q++){
  ...
  if($srtype==1){
    ...
    for($w=1;$w<=$bvaln;$w++){
      ...
      if(mysqli_num_rows($ckinsql)==0){
        //echo "INSERT INTO `labresults` (`refno`, `code`, `label`, `caseno`, `patid`, `labno`, `ap`, `mt`, `pt`, `printbatchno`, `lnvno`, `test`, `preresult`, `result`, `suf`, `nvll`, `nvul`, `unit`, `dispnv`, `remarks`, `dtrec`, `user`) VALUES ('$itmrefno', '$code', '$itmdesc', '$caseno', '$patid', '$labno', '$ap', '$mt', '$pt', '$printbatchno', '$lnvno', '$testname', '$prfn', '$resultn', '$suf', '$nvll', '$nvul', '$unit', '', '$remarks', '".date("Y-m-d H:i:s")."', '$user')<br />";
        mysqli_query($conn,"INSERT INTO `labresults` (`refno`, `code`, `label`, `caseno`, `patid`, `labno`, `ap`, `mt`, `pt`, `printbatchno`, `lnvno`, `test`, `preresult`, `result`, `suf`, `nvll`, `nvul`, `unit`, `dispnv`, `remarks`, `dtrec`, `user`) VALUES ('$itmrefno', '$code', '$itmdesc', '$caseno', '$patid', '$labno', '$ap', '$mt', '$pt', '$printbatchno', '$lnvno', '$testname', '$prfn', '$resultn', '$suf', '$nvll', '$nvul', '$unit', '".mysqli_real_escape_string($conn,$dispnv)."', '$remarks', '".date("Y-m-d H:i:s")."', '$user')");
        //echo "Inserted<br />";
      }
      else{
  ...
  else if(($srtype==2)||($srtype==3)){echo "ASDFG";
  ...
  if($itmerr==0){
    ...
    echo "<span style='color: #0784C0;font-family: arial;font-weight: bold;'>&quot;".$itmdesc."&quot; RESULTS RECORDED SUCCESSFULLY!.</span><br />";
