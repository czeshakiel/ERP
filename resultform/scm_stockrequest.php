...
while($res = $sql->fetch_assoc()){$deptx=$res['supplier']; $reqdate = $res['reqdate'];}
...
<table align="center"><tr>
<td style="font-size: 13px; text-align: center;"><b>STOCK TRANSFER REQUISITION SLIP<b></td>
</tr></table><hr>


<table width="100%" cellpadding="0" cellspacing="0" class="table" style="font-family:Arial; font-size:12px;" border="0">
<tr>
<td style="font-size: 11px; width: 10%;">Requested to: </td>
...
<td style="text-align: center; font-size: 11px; width: 20%;"><b>LAST DATE REQUEST</b></td>
</tr>
';

$i=0;
$result222 = $conn->query("SELECT * FROM purchaseorder WHERE po='$pono' AND `status` NOT LIKE '%Cancel%' AND `status` NOT LIKE '%poreceived%'");
...
$mreqdate="";
$result = $conn->query("SELECT reqdate FROM purchaseorder WHERE code='$code' AND reqdept='$st' AND `status`='received' AND reqdate < '$reqdate' GROUP BY reqno ORDER BY reqdate DESC");
...
<td style="text-align: center; font-size: 11px;">'.$mreqdate.'</td>
</tr>
';
}
