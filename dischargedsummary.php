...
$dd = $conn->query("SELECT po.refno, po.productdesc, po.quantity, h.dosage, h.frequency, h.tam, h.tnn, h.tpm, h.tmn, h.duration,
 h.no FROM productouthm po, homemeds h WHERE po.caseno=h.caseno and po.productcode=h.code and h.caseno='$caseno' group by h.caseno, h.batchno");
while($afetch = $dd->fetch_assoc()){
$no=$afetch['no'];
$refno=$afetch['refno'];
...
<tr>
<td style='font-size:11px;'>$i</td>
<td style='font-size:11px;'><font color='gray'>Item:</font> <b>$afetch[productdesc]</b><br><font color='gray'>Dosage:</font> $afetch[dosage]<br><font color='gray'>Frequency:</font> $afetch[frequency]</td>
...
<td style='font-size:11px;'>
"; if(strpos($refno, "MYHM")!==false){ echo"
<button type=button' class='btn btn-primary btn-sm' style='padding: 5px; font-size: 10px;'"; ?> 
onclick="editval2('<?php echo $productdesc ?>', '<?php echo $quantity ?>', '<?php echo $dosage ?>', '<?php echo $frequency ?>', '<?php echo $tam ?>', '<?php echo $tnn ?>', '<?php echo $tpm ?>', '<?php echo $tmn ?>', '<?php echo $duration ?>', '<?php echo $refno ?>', '<?php echo $no ?>')" 
...
document.getElementById("refnomed").value=val10;
document.getElementById("nomed").value=val11;
var caseno = "<?php echo $caseno ?>";
}
...
var refno = document.getElementById("refnomed").value;
var nox = document.getElementById("nomed").value;
var caseno = "<?php echo $caseno ?>";


$.get("../main/functions.php", {str:str, desc:desc, qty:qty, route:route, frequency:frequency, am:am, nn:nn, pm:pm, mn:mn, duration:duration, caseno:caseno, refno:refno, nox:nox},
function (data){
