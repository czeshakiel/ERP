<?php
$sql = $conn->query("select sum(s.quantity) as soh, r.code, r.description, r.generic from stocktable s, receiving r where s.code=r.code and s.dept='$dept' and r.code='$mycode' group by r.code HAVING soh>0");
...
$sql1 = $conn->query("select * from stocktable where dept='$dept' and code='$code' and  (trantype='charge' or trantype='cash' or trantype='STOCK TRANSFER') and quantity>0 order by datearray");
...
if($reqqty>0){
$ck = $conn->query("select * from purchaseorder where code='$code' and reqdept='$dept' and status='pending' and reqno like '%AUTOREC%'");
if(mysqi_num_rows($ck)>0){
//$conn->query("update");
}else{
$rf= "RN".date("YmdHis");
