					...
					<div class="card-body">
						<table id="employee-table" class="table table-hover align-middle mb-0" style="width: 100%;">
							...
							foreach($purchases as $emp){
								$supplier=str_replace(',','',$emp['supplier']);
								echo "<tr>";
								echo "<td>$x.</td>";
								...
								echo "<td>$emp[user]</td>";?>
								<td><a href="<?=base_url();?>manage_request/<?=$emp['po'];?>/<?=$emp['suppliercode'];?>_<?=$supplier;?>/<?=$emp['reqdept'];?>/<?=$emp['terms'];?>/<?=$emp['trantype'];?>/<?=$emp['reqdate'];?>" class='btn btn-info btn-sm text-white'><i class='icofont-gear'></i> Manage </a></td>
								<?php
								echo "</tr>";
								$x++;
							}
