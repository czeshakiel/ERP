					...
					<div class="card-body">
						...
						<table width="100%" border="0">
							<tr>
								<td><?=$test;?> List</td>
								<td align="right">
									<a href="#" class="btn btn-success btn-sm text-white addMedicoLegal" data-bs-toggle="modal" data-bs-target="#AddMedicoLegal" data-id="<?=$patientidno;?>_<?=$caseno;?>_<?=$test;?>"><i class="icofont-plus-circle"></i> Create New</a>
								</td>
							</tr>
						</table>
						<div class="row table-responsive">
							<table class="table" width="100%" style="font-size: 13px; font-family: Arial;">
								...
								foreach($results as $res){
									$check=$this->Records_model->db->query("SELECT * FROM `collection` WHERE shift='$res[id]' AND `type`='pending'");
									if($check->num_rows()>0){
									...
									echo "<tr>";
									echo "<td>$res[id]</td>";
									echo "<td>$res[medcase]</td>";
									echo "<td>$res[medplace]</td>";
									echo "<td>$res[medtime]</td>";
									echo "<td>$res[meddate]</td>";
									echo "<td>$res[medadvised]</td>";
									echo "<td>$res[medrecommend]</td>";
									echo "<td>";
									...
									echo "<td>";
										if($res['status']=="pending"){
											echo "<a href='#' class='btn btn-outline-warning btn-sm text-warning editMedicoLegal' title='Edit' data-bs-toggle='modal' data-bs-target='#EditMedicoLegal' data-id='".$patientidno."_".$caseno."_".$test."_".$res['id']."'><i class='icofont-ui-edit'></i></a> ";
											?>
											...
											<?php
											echo "<a href='".base_url()."printMedLegalPreview/$patientidno/$caseno/$res[id]' class='btn btn-outline-info btn-sm' target='_blank' title='Preview'><i class='icofont-eye'></i></a> ";
										}else{
											echo "<a href='".base_url()."printMedLegal/$patientidno/$caseno/$res[id]' class='btn btn-outline-info btn-sm' target='_blank' title='Print'><i class='icofont-printer'></i></a> ";
										}
									echo "</td>";
									echo "</tr>";
								}
