...
<div class="body d-flex py-lg-3 py-md-2">
	<div class="container-xxl">
		...
		<?php
			$check=$this->Records_model->db->query("SELECT * FROM collection WHERE datearray='$datearray' AND acctno='WPOS-001' AND refno LIKE '%SPD%'");
			if($check->num_rows()>0){
				$r=$check->row_array();
				if($r['type']=="pending"){
						...
						<table border="0" width="100%">
							<tr>
								...
								<td align="right" width="7%">
									<a href="<?=base_url();?>print_documents_forward/<?=$datearray;?>" target="_blank" class="btn btn-info btn-sm text-white" <?=$view;?>><i class="icofont-print"></i> Print List</a>
								</td>
							</tr>
						</table>
					</div>
