...
//=============================================Credit Check=============================================================
    ...
    <div id="printThis">
    <h4 style="margin-left:90px;">DAILY DISCHARGED REPORT (DEBIT) - <?=$st;?> (<b>RDU</b>)<br>As of <?=$startdate;?></h4>
	<table width="100%" border="1" cellspacing="0" cellpadding="1" style="border-collapse: collapse; font-size: 11px;">
        ...
        $sql=$this->General_model->db->query("SELECT SUM(amount) as amount FROM acctgenledge WHERE caseno='$caseno' AND acctitle LIKE '%DISCOUNT%'");
        if($sql->num_rows() > 0){
            $d=$sql->row_array();
            $disc +=$d['amount'];
        }
    ...
    $sql=$this->General_model->db->query("SELECT SUM(po.hmo) AS amount FROM productout po INNER JOIN admission a ON a.caseno=po.caseno WHERE a.hmo='FIRST BALFOUR' AND a.caseno='$caseno' AND po.productsubtype NOT LIKE '%PROFESSIONAL FEE%'");
        ...
        $sql=$this->General_model->db->query("SELECT SUM(amount) AS amount FROM collection WHERE accttitle LIKE '%AR OFFICE%' AND acctno='$caseno' AND refno NOT LIKE '%LP%'");
        $amount=$sql->row_array();
...
echo "<tr>";
  ...
  if($office1>0){echo "<td align='center' class='font11' width='6%'>OFFICE</td>";}
  ...
  $al2=0;$avega2=0;$chealth2=0;$caritas2=0;$coco2=0;$flexi2=0;$intel2=0;$maxi2=0;$mcl2=0;$mcard2=0;$pcare2=0;$pgicare2=0;$eastwest2=0;$medoc2=0;
        $vcare2=0;$dswd2=0;$lgumak2=0;$lgukid2=0;$pcso2=0;$pepsi2=0;$province2=0;$trade2=0;$tradepf2=0;$pcross2=0;$maip2=0;$insu2=0;$generali2=0;$insulife2=0;$emp2=0;$doc2=0;$cotelco2=0;$tejada2=0;$santos2=0;$caoagdan2=0;$pinol2=0;$balfour2=0;$sss2=0;$discsen2=0;$discnorm2=0;$office2=0;
foreach($body as $item){
	    	$caseno=$item['caseno'];
	    	$patientname=$item['patientname'];
	    	//$coh=$item['amount'];	    	
    ...
    $sql=$this->General_model->db->query("SELECT SUM(po.hmo) AS amount FROM productout po INNER JOIN admission a ON a.caseno=po.caseno WHERE a.hmo='FIRST BALFOUR' AND a.caseno='$caseno' AND po.productsubtype NOT LIKE '%PROFESSIONAL FEE%'");
        ...
        $sql=$this->General_model->db->query("SELECT $qry FROM productout WHERE caseno='$caseno' AND (productsubtype='PHARMACY/SUPPLIES' OR productsubtype='MEDICAL SURGICAL SUPPLIES' OR productsubtype='RESPIRATORY SUPPLIES' OR productsubtype='RDU SUPPLIES' OR productsubtype='RDU-SUPPLIES' OR productsubtype='GENERAL SUPPLIES' OR (productsubtype LIKE '%MISCELLANEOUS%' AND (productdesc NOT LIKE '%MINOR ROOM FEE%' AND productdesc NOT LIKE '%RT FEE%')) OR productsubtype LIKE '%MEDICAL EQUIPMENT%' OR productsubtype='LINENS' OR productsubtype='MEDICAL SUPPLIES' OR (productsubtype='NURSING CHARGES' AND productdesc NOT LIKE '%OSTHEORIZED FEEDING%') OR productsubtype='NURSING-CHARGES' OR productsubtype='OR-CHARGES' OR (productsubtype='NURSING SERVICE FEE' AND productdesc='OR PACK') OR productsubtype='OR/DR SUPPLIES' OR productsubtype='RT ON CALL' OR productsubtype='RT REFERRAL' OR (productsubtype='OR/DR/ER FEE' AND productdesc LIKE '%ARMY NAVY%')) AND productdesc NOT LIKE '%DEATH CERTIFICATE%' AND trantype='charge' AND quantity > 0");
        $amount=$sql->row_array();
        $sup=$amount['amount'];
...
$appfdep=$deposit;
        ...
        //=========================================Credit Check==================================================
        	...
        	$totalamount=$coh+$dep+$disc+$arphic+$arpfphic+$al+$avega+$chealth+$caritas+$coco+$flexi+$intel+$maxi+$mcl+$mcard+$pcare+$vcare+$pcross+$dswd+$lgumak+$lgukid+$pcso+$pepsi+$province+$trade+$tradepf+$maip+$insu+$generali+$insulife+$doc+$emp+$pgicare+$eastwest+$cotelco+$tejada+$santos+$caoagdan+$pinol+$cotelco4+$pcso3+$balfour+$sss+$office;
                $dep1 += $dep;
                $disc1 += $disc;
                $discsen2 += $discsen;
                $discnorm2 += $discnorm;
    ...
    if($office1>0){echo "<td align='right' class='font11' $color>".number_format($office,2)."</td>";}
...
if($office1>0){echo "<td align='right' class='font11' style='font-weight:bold;' $color>".number_format($office2,2)."</td>";}
