<?php
    ...
    ?>
    <div id="printThis">
    <h4 style="margin-left:90px;">DAILY DISCHARGED REPORT (CREDIT) - <?=$st;?> (<b>RDU</b>)<br>As of <?=$startdate;?></h4>
...
$appfdep=$deposit;
    ...
    $totalamount=$meds+$lab+$xray+$utz+$ecg+$eeg+$ct+$echo+$respi+$room+$sup+$rt+$erfee+$admit+$nsfee+$ordr+$oxygen+$cert+$diet+$pt+$ambu+$appfother+$appftrade+$appfphic+$appfdep+$ordrsup;
     $meds2 += $meds;
         ...
         $totalamountDebit=$coh+$dep12+$disc+$arphic+$al+$avega+$chealth+$caritas+$coco+$flexi+$intel+$maxi+$mcl+$mcard+$pcare+$vcare+$pcross+$dswd+$lgumak+$lgukid+$pcso+$pepsi+$province+$trade+$tradepf+$maip+$insu+$generali+$insulife+$doc+$emp+$pgicare+$eastwest+$cotelco+$tejada+$santos+$caoagdan+$pinol+$cotelco4+$pcso3+$balfour+$sss+$medoc+$office;
