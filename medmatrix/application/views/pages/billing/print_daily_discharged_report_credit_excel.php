<?php
    if($button=="1"){
        ?>
        <br>
    <button onclick="tableToExcel('printThisCredit','Discharged_Summary_Debit')">EXPORT CREDIT</button>
    <?php
    }
    ?>
    <?php
    ...
    ?>
    <div id="printThisCredit">
    <h4 style="margin-left:90px;">DAILY DISCHARGED REPORT (CREDIT) - <?=$st;?> (<b><?=$dpt;?></b>)<br>As of <?=$startdate;?></h4>
...
if($type <> "WCOH"){
         ...
         $ordrsup2 += $ordrsup;
         //$refund2 +=$refund;
         $totalamountDebit=$coh+$dep12+$disc+$arphic+$al+$avega+$chealth+$caritas+$coco+$flexi+$intel+$maxi+$mcl+$mcard+$pcare+$vcare+$pcross+$dswd+$lgumak+$lgukid+$pcso+$pepsi+$province+$trade+$tradepf+$maip+$insu+$generali+$insulife+$doc+$emp+$pgicare+$eastwest+$cotelco+$tejada+$santos+$caoagdan+$pinol+$cotelco4+$pcso3+$balfour+$sss+$medoc;
