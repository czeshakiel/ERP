...
<br>
				...
				<b style="font-size:12px;font-family: Times New Roman;">CASH COLLECTION SUMMARY (MEDICINE) - <?=$station;?></b><br>
				As of <?=date('F d, Y',strtotime($rundate));?><br>Shift: <?=$shift;?><br>
											<table width="100%" cellpadding="1" cellspacing="2" border="1" style="font-size:10px; border-collapse: collapse;" >
													...
													$totalamount=0;
															...
															$subtotaldiscount=0;
															$subtotalamount=0;
																	$sqlAmount=mysqli_query($conn,"SELECT acctno,acctname,ofr,description,accttitle,`date`,Dept,paymentTime,SUM(amount) AS amount,SUM(discount) AS discount FROM collection WHERE (accttitle LIKE '%PHARMACY/MEDICINE%' OR accttitle = 'CANCELLED') AND $qry GROUP BY ofr ORDER BY ofr ASC");
