                ...
                <div class="col-lg-12">                  
                    <div class="panel panel-white" id="printableArea" width="100%" style="font-size:12px;">
                            ...
                            <center>
                                <table width="100%" border="0">
                                    <tr>
                                        ...
                                        <td align="center" width="95%"><label style="font-size:18px;font-family: Times New Roman;"><?=$heading;?></label><p><?=$address;?></p><br><label style="font-size: 18px;font-family: Times New Roman;"><b>PROFESSIONAL FEE REPORT</b></label></td>
                                    </tr>
                                </table>
                            </center>
                        </div>
                        <div class="panel-body" id="printThis">
							...
							<b>Collection Type: <u>AP Professional Fee</u></b><br>
							<!--b>Doctor: <u><?=$ap;?></u></b-->
                            <table width="100%" cellpadding="1" cellspacing="0" border="1" style="border-collapse: collapse;">
                                <tr>
									...
									<?php
									$sql="SELECT c.description FROM docfile d INNER JOIN collection c ON c.description=d.`name` WHERE ((c.datearray='$startdate' AND c.paymentTime > '23:00:00') OR (c.datearray='$enddate' AND c.paymentTime <= '23:00:00')) AND (c.accttitle LIKE '%APPF OTHERS%' OR (c.accttitle='PROFESSIONAL FEE' AND c.type='card-Visa')) AND c.amount > 0 AND c.acctno NOT LIKE '%R-%' GROUP BY c.description ORDER BY d.lastname ASC";
								...
								<?php
									$sql="SELECT *,SUM(amount) as amount FROM collection WHERE ((datearray = '$startdate' AND paymentTime > '23:00:00') OR (datearray='$enddate' AND paymentTime <= '23:00:00')) AND (accttitle LIKE '%APPF OTHERS%' OR (accttitle='PROFESSIONAL FEE' AND `type`='card-Visa')) AND amount > 0 AND acctno NOT LIKE '%R-%' AND acctno NOT LIKE '%x%' GROUP BY acctno ORDER BY acctname ASC";
									...
									if(mysqli_num_rows($sqlHospitalBill)>0){
											...
											//if($rundate==$dateto){
												...
												<tr style='font-size:10px;font-family: Times New Roman;'>
													...
													while($pf=mysqli_fetch_array($sqlPF)){
														...
														$sqlAmount=mysqli_query($conn,"SELECT SUM(amount) as amount FROM collection WHERE `description` LIKE '%$pfdesc%' AND acctno='$acctno' AND (accttitle LIKE '%APPF OTHERS%' OR (accttitle='PROFESSIONAL FEE' AND type='card-Visa')) AND ((datearray = '$startdate' AND paymentTime > '23:00:00') OR (datearray='$enddate' AND paymentTime <= '23:00:00')) AND acctno NOT LIKE '%R-%'");
										...
										";
												...
												<tr style='font-size:10px;font-family: Times New Roman;font-weight: bold;'>
													...
													";
													$sql="SELECT SUM(c.amount) as amount,c.description FROM docfile d INNER JOIN collection c ON c.description=d.`name` WHERE ((c.datearray='$startdate' AND c.paymentTime > '23:00:00') OR (c.datearray='$enddate' AND c.paymentTime <= '23:00:00')) AND (c.accttitle LIKE '%APPF OTHERS%' OR (c.accttitle='PROFESSIONAL FEE' AND c.type='card-Visa')) AND c.amount > 0 AND c.acctno NOT LIKE '%R-% ' AND c.acctno NOT LIKE '%x%' GROUP BY c.description ORDER BY d.lastname ASC";
													$sqlPF=mysqli_query($conn,$sql);
													while($pf=mysqli_fetch_array($sqlPF)){
