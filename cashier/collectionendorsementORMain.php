																		...
																		if(mysqli_num_rows($sqlSubAcct)>0){
																			while($sub=mysqli_fetch_array($sqlSubAcct)){
																				...
																				elseif($accttitle=="CASHONHAND" && $subaccttitle=="PROFESSIONAL FEE"){
																					...
																					$sqlAmount=mysqli_query($conn,"SELECT acctname,ofr,description,`date`,Dept,paymentTime,SUM(amount) AS amount,SUM(discount) AS discount,`type`,accttitle FROM collection WHERE accttitle = '$subaccttitle' AND $qry GROUP BY ofr ORDER BY ofr ASC");
																				}
