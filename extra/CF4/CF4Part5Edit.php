...
$count=mysqli_real_escape_string($mycon5,$_GET["count"]);
...
<body>
<?php
$asql=mysqli_query($mycon5,"SELECT * FROM `subjective` WHERE `caseno`='$caseno'");
...
if(empty($_GET["aa"])){$aa="";}
else{$aa=mysqli_real_escape_string($mycon5,$_GET["aa"]);}

$aasql=mysqli_query($mycon1,"SELECT `drugdesc` FROM `phicmedicine` WHERE `drugcode`='$aa'");
...
while($aafetch=mysqli_fetch_array($aasql)){$drugdesc=$aafetch['drugdesc'];}
}

//bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
if(empty($_GET["bb"])){$bb="";}
else{$bb=mysqli_real_escape_string($mycon5,$_GET["bb"]);}

$bbsql=mysqli_query($mycon1,"SELECT `gendesc` FROM `phicmedsgeneric` WHERE `gencode`='$bb'");
...
while($bbfetch=mysqli_fetch_array($bbsql)){$gendesc=$bbfetch['gendesc'];}
}

//ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
if(empty($_GET["cc"])){$cc="";}
else{$cc=mysqli_real_escape_string($mycon5,$_GET["cc"]);}

$ccsql=mysqli_query($mycon1,"SELECT `saltdesc` FROM `phicmedssalt` WHERE `saltcode`='$cc'");
...
while($ccfetch=mysqli_fetch_array($ccsql)){$saltdesc=$ccfetch['saltdesc'];}
}

//ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd
if(empty($_GET["dd"])){$dd="";}
else{$dd=mysqli_real_escape_string($mycon5,$_GET["dd"]);}

$ddsql=mysqli_query($mycon1,"SELECT `strengthdesc` FROM `phicmedsstrength` WHERE `strengthcode`='$dd'");
...
while($ddfetch=mysqli_fetch_array($ddsql)){$strengthdesc=$ddfetch['strengthdesc'];}
}

//eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
if(empty($_GET["ee"])){$ee="";}
else{$ee=mysqli_real_escape_string($mycon5,$_GET["ee"]);}

$eesql=mysqli_query($mycon1,"SELECT `formdesc` FROM `phicmedsform` WHERE `formcode`='$ee'");
...
while($eefetch=mysqli_fetch_array($eesql)){$formdesc=$eefetch['formdesc'];}
}

//fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
if(empty($_GET["ff"])){$ff="";}
else{$ff=mysqli_real_escape_string($mycon5,$_GET["ff"]);}

$ffsql=mysqli_query($mycon1,"SELECT `unitdesc` FROM `phicmedsunit` WHERE `unitcode`='$ff'");
...
while($fffetch=mysqli_fetch_array($ffsql)){$unitdesc=$fffetch['unitdesc'];}
}

//ggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg
if(empty($_GET["gg"])){$gg="";}
else{$gg=mysqli_real_escape_string($mycon5,$_GET["gg"]);}

$ggsql=mysqli_query($mycon1,"SELECT `packagedesc` FROM `phicmedspackage` WHERE `packagecode`='$gg'");
...
while($ggfetch=mysqli_fetch_array($ggsql)){$packagedesc=$ggfetch['packagedesc'];}
}
...
$b=0;
$tp=0;
$bsql=mysqli_query($mycon5,"SELECT * FROM `medicine` WHERE `caseno`='$caseno'");
...
$aaasql=mysqli_query($mycon1,"SELECT `drugdesc` FROM `phicmedicine` WHERE `drugcode`='$pDrugCode'");
...
echo "
        <tr>
          <form name='CF4Part5EditSave' method='post' id='RemoveCF4' action='CF4Part5EditSave.php' ";?> onsubmit="return confirm('Are you sure you want to edit this entry?');" <?php echo ">
";

if($pDrugCode==""){
  $focus="";
echo "
          <td><input type='text' name='pGenericFreeText' id='pGenericFreeText' class='form-control' value='$desc' style='width: 100%; color: #000; margin: 0px 10px 0px 0px; text-transform: uppercase' autocomplete='off' maxlength='500' autofocus /></td>
";
}
else{
  $focus="autofocus";
...
echo "
          <td><input type='text' name='pRoute' id='pRoute' class='form-control' value='$pRoute' style='width: 100px; color: #000; margin: 0px 10px 0px 0px; text-transform: uppercase' maxlength='500' $focus /></td>
          <td><input type='text' name='pFrequencyInstruction' id='pFrequencyInstruction' class='form-control' value='$pInstructionFrequency' style='width: 220px; color: #000; margin: 0px 10px 0px 0px; text-transform: uppercase' maxlength='500' /></td>
          <td><input type='text' name='pQuantity' id='pQuantity' class='form-control' value='$pQuantity' style='width: 60px; color: #000; margin: 0px 10px 0px 0px; text-transform: uppercase' autocomplete='off' maxlength='5' onkeypress='return isNumberKey(event);' /></td>
          <td align='right'><input type='text' name='pTotalPrice' id='pTotalPrice' class='form-control' value='$pTotalAmtPrice' style='width: 80px; color: #000; margin: 0px 10px 0px 0px; text-transform: uppercase' autocomplete='off' maxlength='15' onkeypress='return isNumberWithDecimalKey(event);' />&nbsp;</td>
          <td width='100'><div align='center'><input type='image' name='imageField' src='Resources/Button/Save-Out.png' id='ImageR$b' height='35' onmouseover=MM_swapImage('ImageR$b','','Resources/Button/Save-Over.png',1) onmouseout='MM_swapImgRestore()' /></div></td>
          <input type='hidden' name='caseno' value='$caseno' />
          <input type='hidden' name='source' value='$source' />
...
if($source==1){
echo "
  <form name='CF4Part4' method='get' action='CF4Finalize.php'>
  <tr>
    <td></td>
...
$zsql=mysqli_query($mycon5,"SELECT `pHciCaseNo`, `pHciTransNo` FROM `enlistment` WHERE `caseno`='$caseno'");
