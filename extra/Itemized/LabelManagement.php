...
$a=0;
$asql=mysqli_query($mycon1,"SELECT * FROM `soasetup` ORDER BY CAST(`sort` AS unsigned)");
...
$bsql=mysqli_query($mycon1,"SELECT `productsubtype`, `type`, `producttype`, `terminalname`, `administration`  FROM `soasetupdetails` WHERE `no`='$autono'");
...
if(isset($_GET['addgo'])){
  ...
  mysqli_query($mycon1,"INSERT INTO `soasetup` (`sort`, `label`) VALUES ('$addsort', '$addlabel')");
...
if(isset($_GET['remgo'])){
  ...
  mysqli_query($mycon1,"DELETE FROM `soasetup` WHERE `autono`='$delautono'");
...
if(isset($_GET['edit'])){
  ...
  $csql=mysqli_query($mycon1,"SELECT `label`, `sort` FROM `soasetup` WHERE `autono`='$setautono'");
...
$d=0;
$dsql=mysqli_query($mycon1,"SELECT `refno`, `no`, `productsubtype`, `type`, `producttype`, `terminalname`, `administration` FROM `soasetupdetails` WHERE `no`='$setautono'");
...
if(isset($_GET['update'])){
  ...
  mysqli_query($mycon1,"UPDATE `soasetup` SET `sort`='$setsort', `label`='$setlabel' WHERE `autono`='$fsautono'");
...
if(isset($_GET['delitm'])){
  ...
  mysqli_query($mycon1,"DELETE FROM `soasetupdetails` WHERE `refno`='$setrefno'");
...
if(isset($_GET['additmgo'])){
  ...
  mysqli_query($mycon1,"INSERT INTO `soasetupdetails` (`no`, `productsubtype`, `type`, `producttype`, `terminalname`, `administration`) VALUES ('$addautono', '$addproductsubtype', '$addtype', '$addproducttype', '$addterminalname', '$addadministration')");
  echo "<META HTTP-EQUIV='Refresh'CONTENT='0;URL=LabelManagement.php?edit&autono=$addautono'>";
}


/*$asql=mysqli_query($mycon1,"SELECT `no`, `productsubtype` FROM `soasetupdetails` GROUP BY `no`");
while($afetch=mysqli_fetch_array($asql)){
  $ano=$afetch['no'];
  $aptype=$afetch['productsubtype'];

  $bsql=mysqli_query($mycon1,"SELECT * FROM `soasetup` WHERE `autono`='$ano'");
