...
$asd=0;
$flag=0;
$aeposql=mysqli_query($conn,"SELECT `refno`, `productcode`, `productdesc`, `sellingprice`, `quantity`, `adjustment`, `gross`, `phic`, `phic1`, `hmo`, `excess` FROM `productout` WHERE `caseno`='$caseno' AND `trantype`='charge' AND `gross` > 0 AND `productsubtype` NOT LIKE 'PROFESSIONAL FEE' ORDER BY `datearray`");
while($aepofetch=mysqli_fetch_array($aeposql)){
  ...
  mysqli_query($conn,"UPDATE `productout` SET gross='".(($aesp*$aeqty)-$aeadj)."', phic='0', phic1='0', hmo ='0', excess='".(($aesp*$aeqty)-$aeadj)."' WHERE refno='$aerefno'");
...
echo "
<script>
  alert('Apply excess was successfull!!!');
