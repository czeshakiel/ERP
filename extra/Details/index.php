...
echo "
            <tr>
              ...
              <td class='b1 l1 r2'><div align='right' class='courier s11 black'>&nbsp;".number_format((($pofetch['sp']*$pofetch['qty'])-($pofetch['adj']+$pofetch['hmo']+$pofetch['ph1']+$pofetch['ph2'])),2)."&nbsp;</div></td>
            </tr>
";
        }
      }
...
//ALL OTHERS-------------------------------------------------------------------------------------------------------------------------------
      ...
      $apotph2=0;
      $apothmo=0;
      $aposql=mysqli_query($mycon1,"SELECT `productdesc`, `productcode`, cast(`sellingprice` as decimal(10,2)) AS `sp`, SUM(`quantity`) AS `qty`, SUM(cast(`adjustment` as decimal(10,2))) AS `adj`, SUM(cast(`phic` as decimal(10,2))) AS `ph1`, SUM(cast(`phic1` as decimal(10,2))) AS `ph2`, SUM(cast(`hmo` as decimal(10,2))) AS `hmo`, `remarks` FROM `productout` WHERE `caseno`='$caseno' AND `trantype`='charge' AND `quantity` > 0 AND `productsubtype` NOT LIKE 'PROFESSIONAL FEE' $alloth GROUP BY `productcode`, `sellingprice`, `remarks` ORDER BY `productdesc`");
      while($apofetch=mysqli_fetch_array($aposql)){
        $apotdesc=$apofetch['productdesc'];
        $apotcode=$apofetch['productcode'];
        ...
        $apotrmks=$apofetch['remarks'];

        $apotdesc=str_replace("ams-","",$apotdesc);
        $apotdesc=str_replace("-med","",$apotdesc);
        $apotdesc=str_replace("-sup","",$apotdesc);

        if(($apotcode=="11334620210406")||($apotcode=="210505100721p-50")){
          if($apotrmks!=""){
            $apotdesc=$apotrmks."<br /><span style='color: blue;font-size: 8px;'>&nbsp;(".$apotdesc.")</span>";
          }
        }
...
echo "
            <tr>
              ...
              <td class='b1 l1 r2'><div align='right' class='courier s11 black'>&nbsp;".number_format((($apofetch['sp']*$apofetch['qty'])-($apofetch['adj']+$apofetch['hmo']+$apofetch['ph1']+$apofetch['ph2'])),2)."&nbsp;</div></td>
            </tr>
";
      }
...
";
    ...
    if((stripos($acctitle, "AR ") !== FALSE)&&(stripos($acctitle, " PF") !== FALSE)){
      ...
      if(($acctitle=="AR EMPLOYEE")||($acctitle=="AR EMPLOYEE")){
        $amount=$lessacctfetch['amount'];
        $date=$lessacctfetch['datearray'];
        $removeamt+=$amount;
        $amount=0;
...
$PD_query=mysqli_query($mycon1,"SELECT sum(amount) as amount,acctitle FROM acctgenledge where caseno='$caseno' AND acctitle='DISCOUNT'");
while($PD_fetch=mysqli_fetch_array($PD_query)){
...
echo "
            <tr>
              <td class='t1 b1 l2' colspan='8'><div align='left' class='courier s13 black'>&nbsp;$acctitle</div></td>
              <td class='t1 b1 l1 r2'><div align='right' class='courier s13 black'>&nbsp;".number_format($amount,2)."&nbsp;</div></td>
            </tr>
";
...
echo "
    <tr>
      <td><div align='left'><table border='0' width=70%' cellpadding='0' cellspacing='0'>
        ...
        <tr>
          <td colspan='2'><div align='left' class='arial s14 black'>BILLER: <u><span class='bold'>$name</span></u></div></td>
        </tr>
        <tr>
          <td colspan='2' height='10'></td>
        </tr>
