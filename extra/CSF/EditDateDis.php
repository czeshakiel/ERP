...
<html lang="en">
  <head>
  <title>Discharged Date/Time Information</title>
...
$asql=mysql_query("SELECT datedischarged, timedischarged FROM `dischargedtable` WHERE caseno='$caseno'");
...
if($acount!=0){
  ...
  $displabel="Discharged Date/Time";
  $type=1;
}
else{
  $bsql=mysql_query("SELECT `date`, `time` FROM `tempdatestorage` WHERE `caseno`='$caseno'");
  ...
  if($bcount==0){
    ...
    $datedischarged=$bfetch['date'];
    $timedischarged=$bfetch['time'];
  }
  
  $displabel="Temporary Discharged Date/Time";
  $type=2;
}

if($datedischarged!=""){
  $mm=date("m",strtotime("$datedischarged $timedischarged"));
  $dd=date("d",strtotime("$datedischarged $timedischarged"));
  $yy=date("Y",strtotime("$datedischarged $timedischarged"));
}
else{
  $mm="";
  $dd="";
...
if($timedischarged!=""){
  $hh=date("h",strtotime("$datedischarged $timedischarged"));
  $ii=date("i",strtotime("$datedischarged $timedischarged"));
  $ss=date("s",strtotime("$datedischarged $timedischarged"));
  $aa=date("A",strtotime("$datedischarged $timedischarged"));
}
else{
  $hh="";
  $ii="";
...
echo "
<div align='center'>
  <form name='Save' method='post' action='EditDateDisSave.php' class='form-container-Edit'>
    <table border='0' cellpadding='0' cellspacing='0'>
      <tr>
        <td colspan='3'><div align='center' class='arial s20 blue bold'>$displabel</div></td>
      </tr>
      <tr>
