...
$cfasql=mysqli_query($mycon1,"SELECT hciyes, hcino, disposition, expireddays, timeexpired, transferto, transferadd, reasons, private, nonprivate, doctor1, datesigned1, copay1, doctor2, datesigned2, copay2, doctor3, datesigned3, copay3 FROM claiminfoadd WHERE patientidno='$patientidno' AND caseno='$caseno'");
...
while($fcfetch=mysqli_fetch_array($fcsql)){$fcicdcode=$fcfetch['icdcode'];$fchshare=$fcfetch['hospitalshare'];$fcpshare=$fcfetch['pfshare'];$fcdesc=$fcfetch['description'];}
}
...
while($mcfetch=mysqli_fetch_array($mcsql)){$transmitno1=$mcfetch['transmitno1'];$transmitno2=$mcfetch['transmitno2'];}
...
//NEW PF----------NEW PF----------NEW PF----------NEW PF----------NEW PF----------NEW PF----------NEW PF----------NEW PF----------NEW PF
//PROFESSIONAL FEE-------------------------------------------------------------------------------------------------------------------------
    ...
    if($atcount==1){
      ...
      while($prcofetch=mysqli_fetch_array($prcosql)){
        ...
        //while($yxfetch=mysqli_fetch_array($yxsql)){
          //if($yxfetch['accttitle']!="AR TRADE PF"){
    ...
    if($sucount==1){
      ...
      while($prcsfetch=mysqli_fetch_array($prcssql)){
        ...
        //while($yyfetch=mysqli_fetch_array($yysql)){
          //if($yyfetch['accttitle']!="AR TRADE PF"){
    ...
    if($ancount==1){
      ...
      while($prcafetch=mysqli_fetch_array($prcasql)){
        ...
        //while($yzfetch=mysqli_fetch_array($yzsql)){
          //if($yzfetch['accttitle']!="AR TRADE PF"){
...
$cr1="";
$cr2="";
$fcsql=mysqli_query($mycon1,"SELECT * FROM `finalcaserate` WHERE `caseno`='$caseno' ORDER BY CAST(`autono` AS UNSIGNED)");
...
mysqli_query($mycon1,"SELECT * FROM productoutaddinfo WHERE caseno='$caseno' INTO OUTFILE '/opt/lampp/htdocs/2022codes/eClaims/Charges/$caseno-ai.txt' FIELDS TERMINATED BY '|'");
...
$three=number_format((($conphic+$conphic1+$conhmo+$conexcess)*1),2);
  ...
  else if(($aeptype=="MAMMOGRAPHY")||($aeptype=="EEG")||($aeptype=="HEARTSTATION")){
    if($aeterm!="pending"){
      $hgtot+=($consp*$aeqty);
      $hatot+=$conadj;
    }
...
$pa2sumsql=mysqli_query($mycon6,"SELECT * FROM patientaccountsummary WHERE caseno='$caseno'");
$pa2sumcount=mysqli_num_rows($pa2sumsql);
if($pa2sumcount==0){
mysqli_query($mycon6,"INSERT INTO `kmsci`.`patientaccountsummary` (`caseno`, `hactual`, `hadjustment`, `hgross`, `pfactual`, `pfadjustment`, `pfgross`) VALUES ('$caseno', '$hgtot', '$hatot', '$hntot', '$pgtot', '$patot', '$pntot')");
}
else{
mysqli_query($mycon6,"UPDATE `kmsci`.`patientaccountsummary` SET hactual='$hgtot', hadjustment='$hatot', hgross='$hntot', pfactual='$pgtot', pfadjustment='$patot', pfgross='$pntot' WHERE caseno='$caseno'");
}
