...
$zk=0;
$zksql=mysqli_query($tmpconn,"SELECT * FROM `productout` WHERE `caseno`='$caseno' AND `batchno`='$tick'");
...
echo "
        <form method='post'>
          <table border='0' cellpadding='0' cellspacing='0' >
            <tr>
              <td colspan='6'><div align='left' style='font-family: arial;font-weight: bold;font-size: 12px;color: #008684;'>Items Added</div></td>
            </tr>
            <tr>
...
";
  ...
  while($zkfetch=mysqli_fetch_array($zksql)){
    ...
    $zrsql=mysqli_query($tmpconn,"SELECT `remarks` FROM `labtest` WHERE `refno`='$zkrfn' AND `caseno`='$caseno'");
...
echo "
            <tr>
              ...
              <td class='b1 l1 r2' style='border-radius: 0px;padding: 3px 3px;'><div align='center'><a href='../$aa[3]/$aa[4]&drfn=$zkrfn&itmd'";?> onclick="return confirm('Are you sure you want to remove <?php echo "$zkprd"; ?> from the list?');" <?php echo "><button  type='button' class='btn cancel' style='border-radius: 5px;'>X</button></a></div></td>
            </tr>
";
  }
...
echo "
            <tr>
              <td class='t1' style='border-radius: 0px;padding: 3px;' colspan='6'><div align='right'><button type='submit' name='finalized' class='btn' ";?> onclick="return confirm('Finalize request/s?');" <?php echo ">Finalized</button></div></td>
            </tr>
          </table>
