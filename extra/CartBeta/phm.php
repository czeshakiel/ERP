...
";
  ...
  if($len>1){
    if($ct=="phm"){
      $zxsql=mysqli_query($conn,"SELECT r.`unit`, r.`pnf`, r.`lotno`, r.`testcode`, r.`gtestcode`, r.`code`, r.`description`, r.`generic`, SUM(s.`quantity`) AS `soh`, r.`optset4`, r.`itemname` FROM `stocktable` s, `receiving` r WHERE s.`code`=r.`code` AND ($adu OR r.`unit`='PHARMACY/MEDICINE') AND (r.`description` LIKE '%$searchme%' OR r.`generic` LIKE '$searchme%') AND s.`dept`='$stk' GROUP BY s.`code` ORDER BY r.`pnf` DESC, r.`description` ASC");
          ...
          if($gte==1){
            ...
            if($soh<1){
              ...
              if($pnf!="PNDF"){
                ...
                elseif(strpos($caseno, 'W-')!==false){$qtdis="";}
                else{
                  $qtdis="disabled";
                }
              }
...
echo "
          <tr $pnfwarn>
            ...
            <td class='b1 l1' style='border-radius: 0px;'><div align='left' style='font-family: arial;font-weight: bold;font-size: 14px;color: #000000;padding: 3px 5px;' title='$cod'>$itd"; if($itg!=""){echo " [<span style='color: blue;font-size: 10px;'>".$itg."</span>]";} echo "</div></td>
...
";
              ...
              elseif(strpos($caseno, 'W-')!==false){echo"<td style='border-radius: 0px;'><div align='center' style='padding: 3px 2px;'><button type='submit' name='trantype' value='cash' class='btn cancel'>Cash</button></div></td>";}
              else{
echo "
...
echo "
          <tr>
            <td class='t2' colspan='5'><div align='left' style='color: #FF0000;font-family: arial;font-size: 14px;padding-top: 5px;background-color: #FFFFFF;'>N o t h i n g&nbsp;&nbsp;&nbsp;f o l l o w s . . .</div></td>
          </tr>
        </table>
";
      }
