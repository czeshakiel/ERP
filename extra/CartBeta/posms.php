...
if($fpecount==0){
  ...
  $fpefetch=mysqli_fetch_array($fpesql);
  $pexcess=$fpefetch['gross'];
}
//End Total Excess---------------------------------------------------------------------------------
...
if(($trantype=="charge")&&($pexcess>$ptcl)&&($c[0]=="I")&&($dispense!="on")&&($settrantype!="tpl")){
  ...
  if($kstqty>=$quantity){
    $relqty=$quantity;
    $stsql=mysqli_query($conn,"SELECT `code`, `rrno`, SUM(`quantity`) AS `quantity`, `date`, `po`, `invno`, `unitcost`, `recdqty`, `statquantity`, `lotno`, `terms`, `prodtype1`, `paymentstatus`, `isid`, `prevqty`, `stockalert`, `duedate`, `expiration` FROM `stocktable` WHERE `code`='$code' AND `dept`='$stk' GROUP BY `rrno` HAVING SUM(`quantity`) > 0 ORDER BY `rrno` DESC");
    while($stfetch=mysqli_fetch_array($stsql)){
      ...
      $sdate=$stfetch['date'];
      $spo=$stfetch['po'];
      $sinvno=$stfetch['invno'];
      $sunitcost=$stfetch['unitcost'];
      $srecdqty=$stfetch['recdqty'];
      $sstatquantity=$stfetch['statquantity'];
      $sexpiration=$stfetch['expiration'];
      $slotno=$stfetch['lotno'];
      $sterms=$stfetch['terms'];
      $sprodtype1=$stfetch['prodtype1'];
      $spaymentstatus=$stfetch['paymentstatus'];
      $sisid=$stfetch['isid'];
      $sprevqty=$stfetch['prevqty'];
      $sstockalert=$stfetch['stockalert'];
      $sduedate=$stfetch['duedate'];
      ...
      if($rncount<10){$refno=date("YmdHis")."0000".$rncount;}
      else if($rncount<100){$refno=date("YmdHis")."000".$rncount;}
      else if($rncount<1000){$refno=date("YmdHis")."00".$rncount;}
      else if($rncount<10000){$refno=date("YmdHis")."0".$rncount;}
      else{$refno=date("YmdHis").$rncount;}
      //END GENERATE REFNO-------------------------------------------------------------------------
      ...
      if(($qt>=$relqty)&&($relqty>0)){
        ...
        mysqli_query($insconn,"INSERT INTO `productout` (`refno`, `invno`, `caseno`, `productcode`, `productdesc`, `sellingprice`, `quantity`, `adjustment`, `gross`, `trantype`, `phic`, `hmo`, `excess`, `date`, `status`, `terminalname`, `loginuser`, `batchno`, `producttype`, `productsubtype`, `approvalno`, `referenceno`, `administration`, `shift`, `location`, `datearray`, `phic1`, `cvat`, `cdisc`, `wvat`, `scpwd`, `remarks`, `addons`) VALUES ('$refno', '$invno', '$caseno', '$code', '$descrel', '$price', '$relqty', '$adjustment', '$gross', '$trantype', '$phic1', '$hmo', '$excess', '$date', '$status', '$rrno', '$user', '$batchno', '$tagging', '$unit', '', 'QR-$relqty', '$administration', 'KMSCI', '$dept', '$pdate', '$phic2', '$cvat', '$cdisc', '$wvat', '$scpwd', '$rmks', '$addons')");
        ...
        if(isset($_GET['homemeds'])){
          mysqli_query($conn,"INSERT INTO `productouthm` (`refno`, `invno`, `caseno`, `productcode`, `productdesc`, `sellingprice`, `quantity`, `adjustment`, `gross`, `trantype`, `phic`, `hmo`, `excess`, `date`, `status`, `terminalname`, `loginuser`, `batchno`, `producttype`, `productsubtype`, `approvalno`, `referenceno`, `administration`, `shift`, `location`, `datearray`, `phic1`, `cvat`, `cdisc`, `wvat`, `scpwd`, `remarks`, `addons`) VALUES ('$refno', '$invno', '$caseno', '$code', '$descrel', '$price', '$relqty', '$adjustment', '$gross', '$trantype', '$phic1', '$hmo', '$excess', '$date', '$status', '$rrno', '$user', '$batchno', '$tagging', '$unit', '', 'QR-$relqty', '$administration', 'KMSCI', '$dept', '$pdate', '$phic2', '$cvat', '$cdisc', '$wvat', '$scpwd', '$rmks', '$addons')");
        }

        if($dispense=="on"){
          include("posms-dispensed.php");
          include("posms-stkreq.php");
        }
        ...
        if($relqty!=0){
          ...
          if($code=="202102150002"){$trantype="charge";$status="Approved";}
          mysqli_query($insconn,"INSERT INTO `productout` (`refno`, `invno`, `caseno`, `productcode`, `productdesc`, `sellingprice`, `quantity`, `adjustment`, `gross`, `trantype`, `phic`, `hmo`, `excess`, `date`, `status`, `terminalname`, `loginuser`, `batchno`, `producttype`, `productsubtype`, `approvalno`, `referenceno`, `administration`, `shift`, `location`, `datearray`, `phic1`, `cvat`, `cdisc`, `wvat`, `scpwd`, `remarks`, `addons`) VALUES ('$refno', '$invno', '$caseno', '$code', '$descrel', '$price', '$qt', '$adjustment', '$gross', '$trantype', '$phic1', '$hmo', '$excess', '$date', '$status', '$rrno', '$user', '$batchno', '$tagging', '$unit', '', '$referenceno', '$administration', 'KMSCI', '$dept', '$pdate', '$phic2', '$cvat', '$cdisc', '$wvat', '$scpwd', '$rmks', '$addons')");
