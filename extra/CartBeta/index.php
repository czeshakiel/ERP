...
if(isset($_GET['tick'])){$tick=mysqli_real_escape_string($conn,$_GET['tick']);}else{$tick="";}
...
//auto lab user_login
if((isset($_GET['lin']))&&(isset($_GET['linu']))&&(isset($_GET['linp']))){
  $linu=mysqli_real_escape_string($conn,$_GET['linu']);
  $linp=mysqli_real_escape_string($conn,$_GET['linp']);

  $asql=mysqli_query($conn,"SELECT * FROM `nsauth` WHERE `username`='".base64_decode($linu)."' AND `password`='".base64_decode($linp)."' AND `station`='$dept'");
  $afetch=mysqli_fetch_array($asql);echo $afetch['name'];

  $_SESSION['cun']=base64_encode($linu);
  $_SESSION['cpw']=base64_encode($linp);
  $_SESSION['cac']=base64_encode($afetch['Access']);
  $_SESSION['cnm']=base64_encode($afetch['name']);

  $tick="LXD-".date("YmdHis").rand(1000,9999);

  echo "<META HTTP-EQUIV='Refresh'CONTENT='0;URL=../$aa[3]/".str_replace("&lin&linu=$linu&linp=$linp","",$aa[4])."&tick=$tick'>";
}
...
$comscpwd=1;
$comsp=0;
if(($sethmo!="N/A")||($sethmo!="")){
  $comsql=mysqli_query($conn,"SELECT * FROM `company` WHERE `companyname` LIKE '$sethmo' AND `type`='hmo'");
  $comcount=mysqli_num_rows($comsql);
  if($comcount>0){
    $comfetch=mysqli_fetch_array($comsql);
    $comscpwd=$comfetch['scpwd'];
    $comsp=1;
  }
}
//end know if hmo----------------------------------------------------------------------------------
...
//KNOW CREDIT LEFT---------------------------------------------------------------------------------
  $totgross=0;
  $zasql=mysqli_query($conn,"SELECT `sellingprice`, `quantity`, `adjustment`, `productsubtype` FROM `productout` WHERE `caseno`='$caseno' AND `quantity` > 0  AND `trantype`='charge'");
  while($zafetch=mysqli_fetch_array($zasql)){
    $zapt=$zafetch['productsubtype'];

    if($zapt=="ROOM ACCOMODATION"){$zasp=500;}else{$zasp=$zafetch['sellingprice'];}
...
if($ct=="sot"){
  ...
  $ctlbl="LBAORATORY CART";
  $ctph="Search Laboratory Services...";
  $ctopt="";
}
else if($ct=="phm"){
  $ctlbl="PHARMACY";
          ...
          <table border='0' width='100%' cellpadding='0' cellspacing='0'>
            <tr>
              <td style='padding-bottom: 5px;'><div align='left'>
                <span style='font-family: arial;font-weight: bold;font-size: 16px;color: #000000;'><span style='color: #F5B041;'>&#x263B;</span>PATIENT: <u><span style='color: #0053CB;'>$pn</span></u></span>
              </div></td>
            </tr>
            <tr>
              <td style='padding-bottom: 5px;'><div align='left'>
...
if($adderrch==1){
  $delay=5;
echo "
  <span style='font-family: arial;font-weight: bold;font-size: 16px;color: #FF0000;'>EXCEEDED SET CREDIT LIMIT!!! CANNOT ADD MORE ITEM.</span>
";
}
else if($adderrch==2){
  $delay=5;
echo "
  <span style='font-family: arial;font-weight: bold;font-size: 16px;color: #FF0000;'>PATIENT STATUS IS <span style='color: #F5B041;'><u>YELLOW TAG</u></span>!!! CANNOT ADD ITEM.</span>
";
}
else{
  $delay=2;
