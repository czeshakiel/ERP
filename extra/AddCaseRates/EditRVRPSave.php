...
$editauto=mysqli_real_escape_string($mycon1,$_POST['editauto']);
$caseno=mysqli_real_escape_string($mycon1,$_POST['caseno']);
$icdcode=mysqli_real_escape_string($mycon1,$_POST['icdcode']);
...
mysqli_query($mycon1,"SELECT * FROM finalcaserate WHERE autono='$editauto' INTO OUTFILE '/opt/lampp/htdocs/2021codes/CaseRates/Logs/Edit/$caseno/$icdcode-$editauto-$user-$pdate.txt' FIELDS TERMINATED BY '|'");
...
mysqli_query($mycon1,"UPDATE `finalcaserate` SET `relatedprocedure`='$relatedprocedure', `dateofprocedure`='$dateofprocedure', `laterality`='$laterality' WHERE `autono`='$editauto'");
