...
$tddssql=mysqli_query($mycon1,"SELECT discounttype1, discounttype2, discounttype3, discounttype4, discounttype5, discounttype5o FROM tempdatestorage WHERE caseno='$caseno'");
  ...
  //Attending--------------------------------------------------------------------------------------
    ...
    if($atcount==1){
      ...
      while($prcofetch=mysqli_fetch_array($prcosql)){
        $yxsql=mysqli_query($mycon1,"SELECT `amount`, `accttitle` FROM `collection` WHERE `acctno`='$caseno' AND `description`='".$prcofetch['productdesc']."' AND `accttitle` LIKE '%AR%%PF%'");
  ...
  //Surgeon----------------------------------------------------------------------------------------
    ...
    if($sucount==1){
      ...
      while($prcsfetch=mysqli_fetch_array($prcssql)){
        $yysql=mysqli_query($mycon1,"SELECT `amount`, `accttitle` FROM `collection` WHERE `acctno`='$caseno' AND `description`='".$prcsfetch['productdesc']."' AND `accttitle` LIKE '%AR%%PF%'");
  ...
  //Surgeon----------------------------------------------------------------------------------------
    ...
    if($ancount==1){
      ...
      while($prcafetch=mysqli_fetch_array($prcasql)){
        $yzsql=mysqli_query($mycon1,"SELECT `amount`, `accttitle` FROM `collection` WHERE `acctno`='$caseno' AND `description`='".$prcafetch['productdesc']."' AND `accttitle` LIKE '%AR%%PF%'");
...
$removeamt=0;
$artsql=mysqli_query($mycon1,"SELECT SUM(`amount`) AS `amt`, `datearray`, `refno`, `ofr`,`accttitle` FROM `collection` WHERE `acctno`='$caseno' AND (`accttitle` LIKE 'AR TRADE%%' OR `accttitle` LIKE 'AR EMPLOYEE%%' OR `accttitle` LIKE 'AR DOCTOR%%')AND (`type`='cash-Visa' OR `type`='card-Visa') GROUP BY `ofr`");
...
$lessacct=mysqli_query($mycon1,"SELECT `type` AS `status`, `accttitle` AS `acctitle`, SUM(`amount`) AS `amount`, `datearray`, `refno`, `ofr` FROM `collection` WHERE `acctno`='$caseno' AND `accttitle` LIKE '%AR %' GROUP BY SUBSTRING_INDEX(`accttitle`, '', 3), `datearray`");
while($lessacctfetch=mysqli_fetch_array($lessacct)){
  ...
  if(($acctitle=="AR TRADE")||($acctitle=="AR TRADE PF")){
    $acctitle=str_replace(' PF','',$acctitle);
...
echo "
            <tr>
              <td class='t1 b1 l2'><div align='left' class='times s11 black'>&nbsp;$acctitle - $date</div></td>
              <td class='t1 b1' colspan='6'></td>
              <td class='t1 b1 l1 r2'><div align='right' class='times s11 black'>&nbsp;".number_format($aramt,2)." &nbsp;</div></td>
            </tr>
";
  }
}
...
if(stripos($caseno, "O-") !== FALSE){
  $masql=mysqli_query($mycon1,"SELECT `accttitle`, SUM(`amount`) AS `amount`, `ofr`, `date` FROM `collection` WHERE `acctno`='$caseno' AND ((`description`='HOSPITAL BILL' AND `accttitle`='MEDICAL SURGICAL SUPPLIES') OR (`accttitle`='PROFESSIONAL FEE')) GROUP BY `ofr`");
...
$faddamt=0;
if($shwpf==0){
$lesscollect=mysqli_query($mycon1,"SELECT `accttitle`, SUM(`amount`) AS `amount`, `ofr`, `date` FROM `collection` WHERE `acctno`='$caseno' AND (`accttitle`='CASHONHAND' OR `accttitle`='PROFESSIONAL FEE') AND `type` NOT LIKE 'pending' GROUP BY `ofr`");
while($lesscollectfech=mysqli_fetch_array($lesscollect)){
  $acctitle=$lesscollectfech['accttitle'];
  $amount=$lesscollectfech['amount'];
  $ofr=$lesscollectfech['ofr'];
  ...
  $addamt=0;
  $addconsql=mysqli_query($mycon1,"SELECT `amount` FROM `collection` WHERE `acctno`='$caseno' AND `ofr`='$ofr' AND `description`='HOSPITAL BILL' AND `accttitle`='MEDICAL SURGICAL SUPPLIES' AND `type` NOT LIKE 'pending'");
...
$total_PD=0;
$PD_query=mysqli_query($mycon1,"SELECT * FROM collection where acctno='$caseno' and (accttitle='PATIENTS DEPOSIT' or accttitle='PF DEPOSIT' or accttitle='CASHONHAND' or accttitle='PROFESSIONAL FEE' or accttitle like 'AR %%') and type !='pending' ");
