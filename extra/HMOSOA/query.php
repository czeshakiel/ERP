...
//USERDETAILS
$bsql=mysqli_query($mycon1,"SELECT * FROM `nsauth` WHERE `username`='$user' AND `station`='$dept'");
...
if($c1==""){
  ...
  $zxsql=mysqli_query($mycon1,"SELECT `description` FROM `caserates` WHERE `icdcode`='$crcode1'");
  ...
  if($zxcount==0){
    ...
    $c1disp=" --> ".mysqli_real_escape_string($mycon1,$zxfetch['description']);
  }
}
else{
  $c1disp=" --> ".$c1;
...
if($c2==""){
  $zysql=mysqli_query($mycon1,"SELECT `description` FROM `caserates` WHERE `icdcode`='$crcode2'");
  ...
  if($zycount==0){
    ...
    $c2disp=" --> ".mysqli_real_escape_string($mycon1,$zyfetch['description']);
  }
}
else{
  $c2disp=" --> ".$c2;
...
if($setucount==0){
  $nsusql=mysqli_query($mycon1,"SELECT `name` FROM `nsauth` WHERE `username`='$user' AND `station`='$dept'");
