<?php
    ...
    if($deptval=="CSR2"){$dps1="selected";$dps2="";$dps3="";$dps4="";}
    else if($deptval=="PHARMACY"){$dps1="";$dps2="selected";$dps3="";$dps4="";}
    else if($deptval=="PHARMACY_OPD"){$dps1="";$dps2="";$dps3="selected";$dps4="";}
    else if($deptval=="CSR"){$dps1="";$dps2="";$dps3="";$dps4="selected";}
...
";
    ...
    if(isset($_POST['searchme'])){
      ...
      $zxsql=mysqli_query($conn,"SELECT r.`unit`, r.`pnf`, r.`lotno`, r.`testcode`, r.`gtestcode`, r.`code`, r.`description`, r.`generic`, SUM(s.`quantity`) AS `soh`, r.`optset4`, r.`itemname`, r.`lotno` FROM `stocktable` s, `receiving` r WHERE s.`code`=r.`code` AND (r.`unit` LIKE 'PHARMACY/SUPPLIES' OR r.`unit` LIKE 'MEDICAL SURGICAL SUPPLIES' OR r.`unit` LIKE 'MEDICAL SUPPLIES') AND (r.`description` LIKE '%$searchme%' OR r.`generic` LIKE '$searchme%') AND s.`dept`='$dept' GROUP BY s.`code` ORDER BY r.`description`, r.`generic` ASC");
