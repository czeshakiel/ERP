...
$icdcode=mysqli_real_escape_string($mycon1,$_GET['icdcode']);
$user=base64_decode(mysqli_real_escape_string($mycon1,$_GET['user']));
...
mysqli_query($mycon1,"SELECT * FROM finalcaserate WHERE autono='$autono' INTO OUTFILE '/opt/lampp/htdocs/2021codes/CaseRates/Logs/Edit/$caseno/$icdcode-$autono-$user-$pdate.txt' FIELDS TERMINATED BY '|'");

mysqli_query($mycon1,"UPDATE `finalcaserate` SET `con`='' WHERE `con`='$rvauto'");

mysqli_query($mycon1,"UPDATE `finalcaserate` SET `con`='$rvauto' WHERE `autono`='$autono'");
