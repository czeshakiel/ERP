...
$hcirep=strtoupper(trim(mysql_real_escape_string($_POST['hcirep'])));
$hcidesignation=strtoupper(trim(mysql_real_escape_string($_POST['hcidesignation'])));
...
if($acount==0){
  mysql_query("INSERT INTO `claiminfomoreinfo` (`caseno`, `hcirep`, `hcidesignation`, `hcidatesigned`) VALUES ('$caseno', '$hcirep', '$hcidesignation', '$da')");

  $inlog="$loguser | ('$caseno', '$hcirep', '$hcidesignation', '$da')";
}
else{
  mysql_query("UPDATE `claiminfomoreinfo` SET `hcirep`='$hcirep', `hcidesignation`='$hcidesignation', `hcidatesigned`='$da' WHERE `caseno`='$caseno'");
