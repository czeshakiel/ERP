...
<html lang="en">
  <head>
  <title>Consent to Access Patient Records</title>
...
$cimisql=mysql_query("SELECT `membersuffix`, `carchoose`, `carname`, `cardatesigned`, `carrelation`, `carrelationos`, `carreason`, `carreasonos`, `caruw` FROM claiminfomoreinfo WHERE caseno='$caseno'");
...
if($cimicount==0){
  ...
  while($cimifetch=mysql_fetch_array($cimisql)){
    ...
    $comchoose=$cimifetch['carchoose'];
    $comname=$cimifetch['carname'];
    $comdatesigned=$cimifetch['cardatesigned'];
    $comrelation=$cimifetch['carrelation'];
    $comrelationos=$cimifetch['carrelationos'];
    $comreason=$cimifetch['carreason'];
    $comreasonos=$cimifetch['carreasonos'];
    $comuw=$cimifetch['caruw'];
  }
}
...
else if($setcomchoose=="P"){
  $comnamedisp="$fn $mn $ln $sf";
  
  $scc1="";
  $scc2="";
  $scc3="selected='selected'";
}
...
echo "
<div align='center'>
  <form name='Save' method='post' action='EditConAccSave.php' class='form-container-Edit'>
    <table border='0' cellpadding='0' cellspacing='0'>
      <tr>
        <td colspan='3'><div align='center' class='arial s20 blue bold'>Consent to Access Patient Records</div></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td>
          <select onchange='change_url(this.value);' style='width: 250px;'>
            <option value='EditConAcc.php?caseno=$caseno&patientidno=$patientidno&paymentmode=$paymentmode&setcomchoose=M' $scc1>Member</option>
            <option value='EditConAcc.php?caseno=$caseno&patientidno=$patientidno&paymentmode=$paymentmode&setcomchoose=P' $scc3>Patient</option>
            <option value='EditConAcc.php?caseno=$caseno&patientidno=$patientidno&paymentmode=$paymentmode&setcomchoose=R' $scc2>Representative</option>
          </select>
        </td>
      </tr>
      <tr>
