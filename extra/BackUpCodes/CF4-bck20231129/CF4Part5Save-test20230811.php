...
if(isset($_POST["pRoutecustom"])){
  if(trim(strtoupper(mysqli_real_escape_string($mycon5,$_POST["pRoutecustom"])))!=""){
    $pRoute=trim(strtoupper(mysqli_real_escape_string($mycon5,$_POST["pRoutecustom"])));
  }
}
$pFrequencyInstruction=strtoupper(mysqli_real_escape_string($mycon5,$_POST["pFrequencyInstruction"]));
if(isset($_POST["pFrequencyInstructioncustom"])){
  if(strtoupper(mysqli_real_escape_string($mycon5,$_POST["pFrequencyInstructioncustom"]))!=""){
    $pFrequencyInstruction=strtoupper(mysqli_real_escape_string($mycon5,$_POST["pFrequencyInstructioncustom"]));
  }
}
...
$pGenericFreeText=str_replace("  "," ",$pGenericFreeText);
$pGenericFreeText=str_replace("  "," ",$pGenericFreeText);
$pGenericFreeText=trim($pGenericFreeText);

$pRoute=str_replace("`","",$pRoute);
$pRoute=str_replace("%","PERCENT",$pRoute);
$pRoute=str_replace("<","LESS THAN",$pRoute);
$pRoute=str_replace(">","MORE THAN",$pRoute);
$pRoute=str_replace("&","AND",$pRoute);
$pRoute=str_replace(":","",$pRoute);
$pRoute=str_replace("  "," ",$pRoute);
$pRoute=str_replace("  "," ",$pRoute);
$pRoute=trim($pRoute);
...
$pFrequencyInstruction=str_replace("  "," ",$pFrequencyInstruction);
$pFrequencyInstruction=str_replace("  "," ",$pFrequencyInstruction);
$pFrequencyInstruction=trim($pFrequencyInstruction);
...
mysqli_query($mycon1,"UPDATE `poaddon` SET `status`='added' WHERE `caseno`='$caseno' AND `drugcode`='$pDrugCode'");
...
echo "<META HTTP-EQUIV='Refresh'CONTENT='10;URL=CF4Part5-test20230811.php?caseno=$caseno&source=$source'>";
}
else{
echo "
<table style='height:100%;width:100%; position: absolute; top: 0; bottom: 0; left: 0; right: 0;' border='0' cellpadding='0' cellspacing='0'>
...
echo "<META HTTP-EQUIV='Refresh'CONTENT='1;URL=CF4Part5-test20230811.php?caseno=$caseno&source=$source&aa=$pDrugCode&bb=$pGeneric&cc=$pSalt&dd=$pStrength&ee=$pForm&ff=$pUnit&gg=$pPackage'>";
}
}
else{
