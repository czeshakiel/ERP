<?php
  //AUTO MEDS------------------------------------------------------------------------------------------------------------------------------------------
  $medinum=0;
  $medisql=mysqli_query($mycon1,"SELECT `productdesc`, `productcode`, SUM(`quantity`) AS `quantity`, `sellingprice`, SUM(`adjustment`) AS `adjustment`, SUM((`quantity`*`sellingprice`)-`adjustment`) AS `netmed`, `drugcode` FROM `poaddon` WHERE `caseno`='$caseno' AND `status`='pending' AND `drugcode` NOT LIKE '' GROUP BY `drugcode` ORDER BY `refno`");
