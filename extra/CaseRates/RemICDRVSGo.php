...
$autono=mysqli_real_escape_string($conn,$_POST['autono']);
$user=base64_decode(mysqli_real_escape_string($conn,$_POST['user']));

$asql=mysqli_query($conn,"SELECT `icdcode` FROM `finalcaserate` WHERE `autono`='$autono'");
...
mysqli_query($conn,"SELECT * FROM finalcaserate WHERE autono='$autono' INTO OUTFILE '/opt/lampp/htdocs/2021codes/CaseRates/Logs/Delete/$caseno/$icdcode-$autono-$user-$pdate.txt' FIELDS TERMINATED BY '|'");
...
mysqli_query($conn,"DELETE FROM `finalcaserate` WHERE `autono`='$autono'");
