...
$idno=mysqli_real_escape_string($conn,$_GET['idno']);
$frm=mysqli_real_escape_string($conn,$_GET['frm']);
$rvauto=mysqli_real_escape_string($conn,$_GET['rvauto']);

$asql=mysqli_query($conn,"SELECT `icdcode`, `category`, `groupid` FROM `caserates` WHERE `idno`='$idno'");
...
$bsql=mysqli_query($conn,"SELECT `autono` FROM `finalcaserate` WHERE `caseno`='$caseno'");
$bcount=mysqli_num_rows($bsql);

if($bcount!=0){
//1st caserate-----------------------------------
  $csql=mysqli_query($conn,"SELECT `autono`, `con` FROM `finalcaserate` WHERE `caseno`='$caseno' AND `level`='primary'");
  $ccount=mysqli_num_rows($csql);
//2nd caserate-----------------------------------
  $dsql=mysqli_query($conn,"SELECT `autono`, `con` FROM `finalcaserate` WHERE `caseno`='$caseno' AND `level`='secondary'");
...
//SPOC Warning-----------------------------------
$esql=mysqli_query($conn,"SELECT `patientidno` FROM `admission` WHERE `caseno`='$caseno'");
