...
$icdcode=mysqli_real_escape_string($conn,$_GET['icdcode']);
$user=base64_decode(mysqli_real_escape_string($conn,$_GET['user']));
$autono=mysqli_real_escape_string($conn,$_GET['autono']);
...
mysqli_query($conn,"SELECT * FROM finalcaserate WHERE autono='$autono' INTO OUTFILE '/opt/lampp/htdocs/2021codes/CaseRates/Logs/Edit/$caseno/$icdcode-$autono-$user-$pdate.txt' FIELDS TERMINATED BY '|'");

mysqli_query($conn,"UPDATE `finalcaserate` SET `con`='' WHERE `con`='$rvauto'");

mysqli_query($conn,"UPDATE `finalcaserate` SET `con`='$rvauto' WHERE `autono`='$autono'");
