...
$cfasql=mysqli_query($conn,"SELECT `doctor4`, `datesigned4`, `copay4`, `doctor5`, `datesigned5`, `copay5`, `doctor6`, `datesigned6`, `copay6` FROM `claiminfoadd2` WHERE `caseno`='$caseno'");
...
if($cfacount==0){
  ...
  $doctor4=$cfafetch['doctor4'];
  $datesigned4=$cfafetch['datesigned4'];
  $copay4=$cfafetch['copay4'];
  $doctor5=$cfafetch['doctor5'];
  $datesigned5=$cfafetch['datesigned5'];
  $copay5=$cfafetch['copay5'];
  $doctor6=$cfafetch['doctor6'];
  $datesigned6=$cfafetch['datesigned6'];
  $copay6=$cfafetch['copay6'];
}

$dcname4=$doctor4;echo $docname4;

mysqli_query($conn,"SET NAMES utf8");
$dfsql=mysqli_query($conn,"SELECT `phicacc` FROM `docfile` WHERE `name` like '$dcname4'");
...
mysqli_query($conn,"SET NAMES utf8");
$df2sql=mysqli_query($conn,"SELECT `phicacc` FROM `docfile` WHERE `name`='$dcname5'");
...
mysqli_query($conn,"SET NAMES utf8");
$df3sql=mysqli_query($conn,"SELECT `phicacc` FROM `docfile` WHERE `name`='$dcname6'");
...
if(getBrowser()!="Firefox"){
  if(file_exists("Sig/".$phicac4."_".$dnum.".png")){
    echo "<img src='Sig/".$phicac4."_".$dnum.".png' style='position: absolute;left: 160px;top: 1220px;width: auto;height: 75px;' alt='$phicac4'  />";
  }

  if(file_exists("Sig/".$phicac5."_".$dnum.".png")){
    echo "<img src='Sig/".$phicac5."_".$dnum.".png' style='position: absolute;left: 270px;top: 1250px;width: auto;height: 75px;' alt='$phicac5'  />";
  }

  if(file_exists("Sig/".$phicac6."_".$dnum.".png")){
    echo "<img src='Sig/".$phicac6."_".$dnum.".png' style='position: absolute;left: 160px;top: 1290px;width: auto;height: 75px;' alt='$phicac6'  />";
  }
}
...
echo "
    ...
    <table width='750' border='0' cellpadding='0' cellspacing='0'>
      <tbody>
        <tr>
          <th style='border-bottom: 2px solid black;' scope='col' class='tahoma s12 black bold ' height='26' align='center' valign='middle'>PART IV - HEALTH CARE PROFESSIONAL INFORMATION</th>
        </tr>
        <tr>
          <td height='3'></td>
        </tr>
        <tr>
          <td $cursty onclick='myFunction81()'><table border='0' width='100%' cellpadding='0' cellspacing='0'>
            <tr>
              <td width='7'></td>
...
function myFunction81() {
  window.open('AddEditProfDetailsMore.php?caseno=$caseno', '_blank', 'toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=600,width=500,height=700');
}
</script>
";
?>
