...
mysql_select_db($cuz->getDB());
mysql_query('SET NAMES utf8');
$cfsql=mysql_query("SELECT identificationno, UPPER(lastname) AS lastname, UPPER(firstname) AS firstname, UPPER(middlename) AS middlename FROM claiminfo WHERE caseno='$caseno'");
...
mysql_select_db($cuz->getDB());
mysql_query('SET NAMES utf8');
$cfasql=mysql_query("SELECT hciyes, hcino, disposition, expireddays, timeexpired, transferto, transferadd, reasons, private, nonprivate, doctor1, datesigned1, doctor2, datesigned2, copay2, doctor3, datesigned3, copay3 FROM claiminfoadd WHERE caseno='$caseno'");
...
$hsql=mysql_query("SELECT * FROM `claiminfoadd2` WHERE `caseno`='$caseno'");
$hcount=mysql_num_rows($hsql);
if($hcount!=0){
echo "
<br />
<br />
...
$h2sql=mysql_query("SELECT * FROM `claiminfoadd2` WHERE `caseno`='$caseno' AND (`doctor7` NOT LIKE '' OR `doctor8` NOT LIKE '' OR `doctor9` NOT LIKE '')");
$h2count=mysql_num_rows($h2sql);
if($h2count){
include("CSF6B.php");
}
