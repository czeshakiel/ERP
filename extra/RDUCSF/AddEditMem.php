...
$spatid=$adfetch['patientidno'];

if($spatid!=$patientidno){
  $patientidno=$spatid;
}
...
if($cfcount==0){
  $cf2sql=mysqli_query($conn,"SELECT `identificationno`, UPPER(`lastname`) AS `lastname`, UPPER(`firstname`) AS `firstname`, UPPER(`middlename`) AS `middlename` FROM `claiminfo` WHERE `caseno` NOT LIKE '$caseno' AND `patientidno`='$patientidno' AND `identificationno` NOT LIKE '' AND `lastname` NOT LIKE ''");
  ...
  mysqli_query($conn,"UPDATE `claiminfo` SET `patientidno`='$patientidno' WHERE `caseno`='$caseno'");
