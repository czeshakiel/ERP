...
$dept=mysqli_real_escape_string($mycon1,$_POST['dept']);
$ipass=mysqli_real_escape_string($mycon1,$_POST['enterpassword']);
$nursename=mysqli_real_escape_string($mycon1,$_POST['user']);
$ticket=mysqli_real_escape_string($mycon1,$_POST['ticket']);
$pckgno=mysqli_real_escape_string($mycon1,$_POST['pckgno']);
$pn=mysqli_real_escape_string($mycon1,$_POST['pn']);
$remarks=strtoupper(mysqli_real_escape_string($mycon1,$_POST['remarks']));
$disrel=mysqli_real_escape_string($mycon1,$_POST['disrel']);
$prc=mysqli_real_escape_string($mycon1,$_POST['prc']);
$frm=mysqli_real_escape_string($mycon1,$_POST['frm']);
$trantype=mysqli_real_escape_string($mycon1,$_POST['trantype']);
$station=mysqli_real_escape_string($mycon1,$_POST['station']);
...
$zysql=mysqli_query($mycon1,"SELECT * FROM `admission` WHERE `caseno`='$caseno' AND `membership`='phic-med'");
$zycount=mysqli_num_rows($zysql);

$bsql=mysqli_query($mycon1,"SELECT * FROM `packagedetails` WHERE `pckgno`='$pckgno' ORDER BY CAST(`price` AS DECIMAL(10,2)) DESC");
...
while($bfetch=mysqli_fetch_array($bsql)){
  ...
  $csql=mysqli_query($mycon1,"SELECT `unit` FROM `receiving` WHERE `code`='$cd'");
  $cfetch=mysqli_fetch_array($csql);
  $unit=$cfetch['unit'];

  $zsql=mysqli_query($mycon1,"SELECT `patientidno` FROM `admission` WHERE `caseno`='$caseno'");
  $zfetch=mysqli_fetch_array($zsql);
  $patientidno=$zfetch['patientidno'];

  $ysql=mysqli_query($mycon1,"SELECT `patientname` FROM `patientprofile` WHERE `patientidno`='$patientidno'");
  ...
  //POSPHARMA
    echo "
      <iframe src='../Cart2021/packagepharma.php?caseno=$caseno&code=$cd&qty=1&trantype=$trantype&toh=PACKAGE&station=$station&tick=PHARMACY$ticket&stk=PHARMACY&sprel=$pr&referenceno=$pckgno&packrno=$refno&itd=$apdis&srno=$refno".$addon."' height='0' width='0' style='border: none;'></iframe>
    ";
  }
...
echo "
<div align='left' class='arial16bluebold'><td><input type='button' onclick=MM_openBrWindow('http://192.168.0.100:100/cgi-bin/printallsup2.cgi?ticketno=LXD-$ticket&amp;caseno=$caseno','','toolbar=yes,location=yes,status=yes,menubar=yes,scrollbars=yes,resizable=yes,width=800,height=800') value='CHARGE SLIP NO. $tick' /></td><br />PACKAGE ADDED</div>
<br />
<div align='left' class='arial16bluebold'>
  <form name='back' action='../Package2022/' method='post'>
