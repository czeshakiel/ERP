...
$ipass=mysqli_real_escape_string($mycon1,$_POST['ipass']);
...
$dis=mysqli_real_escape_string($mycon1,$_POST['dis']);
...
$asql=mysqli_query($mycon1,"SELECT p.`senior` FROM `admission` a, `patientprofile` p WHERE `caseno`='$caseno' AND a.`patientidno`=p.`patientidno`");
...
$xsql=mysqli_query($mycon1,"SELECT `pckgno`, `packagename`, `price`, `dept`, `discount` FROM `packagelist` WHERE `pckgno`='$pckgno'");
...
<div align='left' class='arial16bluebold'>
  <form name='back' action='../Package/' method='post'>
