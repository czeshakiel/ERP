        ...
        <tr>
          ...
          <td width='42%'><table width='100%' border='0' cellpadding='0' cellspacing='0'>
            <tr>
              ...
              <td width='auto' class='table1Bottom'><div align='left' class='times9black'>&nbsp;".date("M d, Y",strtotime($_GET['dateadmit']))." ".date('h:i A',strtotime($_GET['timeadmit']))."&nbsp;</div></td>
            </tr>
          </table></td>
          <td width='4%'></td>
        </tr>
...
$dtsql=mysql_query("SELECT dateadmitted, timeadmitted FROM admission WHERE caseno='$caseno'");
...
$tdsdate=$tdsfetch['dateadmitted'];
//$tdsdate=str_replace("_","-",$tsdate);
$tdstime=$tdsfetch['timeadmitted'];
}
...
$datedischarged=$dtfetch['dateadmitted'];
$datedischarged=str_replace("_","-",$datedischarged);
$timedischarged=$dtfetch['timeadmitted'];
}

echo "
                &nbsp;".date("M d, Y",strtotime($datedischarged))." ".date('h:i A',strtotime($timedischarged))."&nbsp;
";
}
...
$phic1=0;
$excess=0;
while($poutdfetch=mysql_fetch_array($poutdsql)){$poutdadjtot+=$poutdfetch['adjustment']*$quantity;$poutdgrosstot+=$poutdfetch['gross']*$quantity;$actual+=($poutdfetch['sellingprice']*$quantity);$hmo+=$poutdfetch['hmo']*$quantity;$phic+=$poutdfetch['phic']*$quantity;$phic1+=$poutdfetch['phic1']*$quantity;$excess+=$poutdfetch['excess']*$quantity;}
...
$pfadactual2+=($pfadfetch2['sellingprice']*$quantity);
$pfadadj2+=$pfadfetch2['adjustment']*$quantity;
$pfadgross2+=$pfadfetch2['gross']*$quantity;
$pfadhmo2+=$pfadfetch2['hmo']*$quantity;
$pfadphic2+=$pfadfetch2['phic']*$quantity;
$pfadphic12+=$pfadfetch2['phic1']*$quantity;
$pfadexcess2+=$pfadfetch2['excess']*$quantity;
}*/
//---------------------------------------------------------------------------------------------------------------------
...
$phic1=0;
$excess=0;
while($poutasddfetch=mysql_fetch_array($poutasddsql)){$poutasddadjtot+=$poutasddfetch['adjustment']*$quantity;$poutasddgrosstot+=$poutasddfetch['gross']*$quantity;$actual+=($poutasddfetch['sellingprice']*$quantity);$hmo+=$poutasddfetch['hmo']*$quantity;$phic+=$poutasddfetch['phic']*$quantity;$phic1+=$poutasddfetch['phic1']*$quantity;$excess+=$poutasddfetch['excess']*$quantity;}
...
$htot=((($totactual-$adjsubtot-($agamount+$hmotot+$hdamt+$hd2amt+$hd3amt+$hd4amt))-$fchshare-$fcpshare)-$sechshare);
...
echo "
        <tr>
          ...
          <td><div align='right' class='times13blackbold'>&nbsp;".number_format($totactual-$adjsubtot-($hmotot+$agamount+$hdamt+$hd2amt+$hd3amt+$hd4amt)-$phictot-$phic1tot,2,'.',',')."&nbsp;</div></td>
        </tr>
";
...
$b=0;
$popfsql=mysql_query("SELECT productdesc, producttype FROM productout WHERE caseno='$caseno' AND trantype='charge' AND (producttype='IPD attending' OR producttype='IPD surgeon' OR producttype='IPD anesthesiologist' OR producttype='RDU') AND productsubtype='PROFESSIONAL FEE' GROUP BY productdesc");
...
$pfgross=$asafetch['gross']*$quantity;
$pfadj=$asafetch['discount']*$quantity;
$pfphic1=$asafetch['p1']*$quantity;
$pfphic2=$asafetch['p2']*$quantity;
$pfhmo=$asafetch['hmo']*$quantity;
$pfexc=$asafetch['exc']*$quantity;
...
//Start Other Doctors
$otdsql=mysql_query("SELECT (sellingprice*quantity) AS gross, adjustment AS discount, phic AS p1, phic1 AS p2, hmo AS hmo, excess AS exc FROM productout WHERE caseno='$caseno' AND productdesc NOT LIKE '$doctor' AND trantype='charge' AND producttype NOT LIKE 'IPD attending' AND producttype NOT LIKE 'IPD surgeon' AND producttype NOT LIKE 'IPD anesthesiologist' AND producttype NOT LIKE 'IPD admitting' AND productsubtype='PROFESSIONAL FEE'");
...
echo "
        <tr>
          ...
          <td><div align='right' class='times13blackbold'>&nbsp;".number_format($totfingross-$totfinadj-$totfinhmo-$totfinphic1-$totfinphic2,2,'.',',')."&nbsp;</div></td>
        </tr>
        <tr>
          ...
          <td><div align='right' class='times13blackbold'>&nbsp;".number_format($totactual+$totfingross-($adjsubtot+$totfinadj)-($hmotot+$totfinhmo+$agamount+$hdamt+$hd2amt+$hd3amt+$hd4amt)-($totfinphic1+$phictot)-($totfinphic2+$phic1tot),2,'.',',')."&nbsp;</div></td>
        </tr>

";
$totalbill=$totactual+$totfingross-($adjsubtot+$totfinadj)-($hmotot+$totfinhmo+$agamount+$hdamt+$hd2amt+$hd3amt+$hd4amt)-($totfinphic1+$phictot)-($totfinphic2+$phic1tot);
   ...
   $lesscollect=mysql_query("SELECT accttitle,sum(amount) as amount,ofr,date FROM collection where acctno='$caseno' and (accttitle='CASHONHAND' or accttitle='PROFESSIONAL FEE') and type !='pending' group by ofr");
        ...
        $ofr=$lesscollectfech['ofr'];
          ...
          <tr>
                                    ...
                                    <td align='right' class='times10black' colspan='7'>".number_format($amount*$quantity,2)."</td>
                                    </tr>
                                    ";
...
//start-- sum of the total Less (AR and Patient's Deposit)
        ...
        $total_less= $total_AR + $total_PD;
          echo "<tr>
                                    ...
                                    <td align='right' class='times13blackbold' >".number_format($total_less*$quantity,2)."</td>
                                    </tr>
                                    ";
//end-- sum of the total Less (AR and Patient's Deposit)
            $net=($totalbill-($total_less*$quantity))*$quantity;
          echo "<tr>
