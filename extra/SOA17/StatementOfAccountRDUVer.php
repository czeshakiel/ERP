...
$fingross=($pfgross+($otdgross-($sdgross+$adgross)))*0;
$finadj=($pfadj+($otdadj-($sdadj+$adadj)))*0;
$finnet=$fingross-$finadj;
$finphic1=($pfphic1+($otdphic1-($sdphic1+$adphic1)))*0;
$finphic2=($pfphic2+($otdphic2-($sdphic2+$adphic2)))*0;
$finhmo=($pfhmo+($otdhmo-($sdhmo+$adhmo)))*0;
$finexc=($pfexc+($otdexc-($sdexc+$adexc)))*0;
...
echo "
        ...
        ";
                       // $lessacct=mysql_query("SELECT * FROM acctgenledge where caseno='$caseno'");
                        $lessacct=mysql_query("SELECT SUBSTRING_INDEX(acctitle, ' ', 2) as acctitle,sum(amount) as amount,date FROM acctgenledge where caseno='$caseno' and  (acctitle like '%AR%' or acctitle='DISCOUNT') group by SUBSTRING_INDEX(acctitle, ' ', 2) ");
...
//start-- sum of the total Less (AR and Patient's Deposit)
                    ...
                    while($AR_fetch=mysql_fetch_array($AR_query))
                            {
                             ...
                             $total_AR +=$amount;
                              ...
                              $total_DISC=0;
                              $AR_query=mysql_query("SELECT * FROM acctgenledge where caseno='$caseno' and (acctitle='DISCOUNT')");
                                        while($AR_fetch=mysql_fetch_array($AR_query))
                                                {
                                                 $acctitle=$AR_fetch['acctitle'];
                                                 $amount=$AR_fetch['amount'];
        ...
        $total_less= $total_AR + $total_PD + $total_DISC;
          echo "<tr>
