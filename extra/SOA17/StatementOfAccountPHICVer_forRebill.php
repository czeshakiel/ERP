...
$admsql=mysql_query("SELECT patientidno, membership, paymentmode, room, UPPER(street) AS street, UPPER(barangay) AS barangay, UPPER(municipality) AS municipality, UPPER(province) AS province,UPPER(pastmed) AS pastmed, zipcode, UPPER(initialdiagnosis) AS initialdiagnosis, UPPER(finaldiagnosis) AS finaldiagnosis, UPPER(ap) AS ap, timeadmitted, dateadmit, contactno, employerno, hmomembership, hmo, status FROM admission WHERE caseno='$caseno'");
...
if(($pstype=='MEDICAL SUPPLIES')||($pstype=='PHARMACY/MEDICINE')||($pstype=='SALES-SUPPLIES')||($pstype=="GENERAL SUPPLIES")||($pstype=='PHARMACY/SUPPLIES')||($pstype=='MEDICAL SURGICAL SUPPLIES')){
$poutdsql=mysql_query("SELECT refno, sellingprice, quantity, adjustment, gross, hmo, phic, phic1, excess FROM productout WHERE caseno='$caseno' AND quantity > 0 AND trantype='charge' AND productsubtype='$pstype' AND administration NOT LIKE 'pending'");
...
$phic1=0;
$excess=0;
while($poutdfetch=mysql_fetch_array($poutdsql)){
  if(($pstype=='MEDICAL SUPPLIES')||($pstype=='PHARMACY/MEDICINE')||($pstype=='SALES-SUPPLIES')||($pstype=='PHARMACY/SUPPLIES')||($pstype=='MEDICAL SURGICAL SUPPLIES')){
    $cretsql=mysql_query("SELECT *  FROM `productreturn` WHERE `refno1`='".$poutdfetch['refno']."' AND `trantype`='pending'");
    if(mysql_num_rows($cretsql)>0){$cret+=1;}
  }
  $poutdadjtot+=$poutdfetch['adjustment'];$poutdgrosstot+=$poutdfetch['gross'];$actual+=($poutdfetch['sellingprice']*$poutdfetch['quantity']);$hmo+=$poutdfetch['hmo'];$phic+=$poutdfetch['phic'];$phic1+=$poutdfetch['phic1'];$excess+=$poutdfetch['excess'];
}
...
if(($pstype!="PHARMACY/SUPPLIES")&&($pstype!="MEDICAL SURGICAL SUPPLIES")&&($pstype!="GENERAL SUPPLIES")){
echo "
        <tr>
...
//MISC
else if(($pstype=='ADMISSION FEE')||($pstype=="ER FEE")||($pstype=="MEDICAL EQUIPMENT")||($pstype=="MISCELLANEOUS")||($pstype=="OR-CHARGES")||($pstype=="OR/DR SUPPLIES")||($pstype=="OR/DR/ER FEE")||($pstype=='ADMITTING FEE')||($pstype=='MISCELLANEOUS CR')||($pstype=='NURSING CHARGES')||($pstype=='NURSING SERVICE FEE')||($pstype=='NURSING_CHARGES')||($pstype=='LINENS')||($pstype=='LAUNDRY SUPPLIES') ||($pstype=='OTHER INCOME')){
...
$b=0;
$popfsql=mysql_query("SELECT productdesc, producttype FROM productout WHERE caseno='$caseno' AND trantype='charge' AND (producttype='IPD attending' OR producttype='IPD surgeon' OR producttype='IPD anesthesiologist' OR producttype='ON CALL') AND productsubtype='PROFESSIONAL FEE' GROUP BY productdesc ORDER BY FIELD(`producttype`, 'IPD attending', 'IPD surgeon', 'IPD anesthesiologist', 'ON CALL')");
...
/*                 $lessacct=mysql_query("SELECT type as status,accttitle as acctitle,sum(amount) as amount,datearray,refno,ofr FROM collection where acctno='$caseno'  and accttitle like '%AR %'   group by SUBSTRING_INDEX(accttitle, ' ', 2),ofr");
...
echo "
<br />
<div align='center' style='font-family: Arial;font-size: 30px;color: #FF0000;font-weight: bold;'>WARNING!!! PATIENT STILL HAS PENDING MEDICINES OR SUPPLIES FOR RETURN. $cret ITEM/S PENDING FOR RETURN.</div>
";
}
...
echo "
<br />
<div align='center' style='font-family: Arial;font-size: 30px;color: #FF0000;font-weight: bold;'>WARNING!!! PATIENT STILL HAS PENDING MEDICINES OR SUPPLIES. $pencount</div>
";
}
