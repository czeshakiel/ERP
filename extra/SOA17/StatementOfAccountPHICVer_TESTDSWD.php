  ...
  //Attending--------------------------------------------------------------------------------------
    ...
    if($atcount==1){
      ...
      while($prcofetch=mysqli_fetch_array($prcosql)){
        $yxsql=mysqli_query($mycon1,"SELECT `amount` FROM `collection` WHERE `acctno`='$caseno' AND `description`='".$prcofetch['productdesc']."' AND `accttitle` LIKE '%AR%%PF%'");
        while($yxfetch=mysqli_fetch_array($yxsql)){
          $yxamnt+=$yxfetch['amount'];
        }

        $pragr+=($prcofetch['sellingprice']*$prcofetch['quantity']);$praad+=$prcofetch['adjustment'];$prap1+=$prcofetch['phic'];$prap2+=$prcofetch['phic1'];$prahm+=($prcofetch['hmo']+$yxamnt);$praex+=($prcofetch['excess']-$yxamnt);
      }
    }
  //End Attending----------------------------------------------------------------------------------
  }
  ...
  //Surgeon----------------------------------------------------------------------------------------
    ...
    if($sucount==1){
      ...
      while($prcsfetch=mysqli_fetch_array($prcssql)){
        $yysql=mysqli_query($mycon1,"SELECT `amount` FROM `collection` WHERE `acctno`='$caseno' AND `description`='".$prcsfetch['productdesc']."' AND `accttitle` LIKE '%AR%%PF%'");
  ...
  //Surgeon----------------------------------------------------------------------------------------
    ...
    if($ancount==1){
      ...
      while($prcafetch=mysqli_fetch_array($prcasql)){
        $yzsql=mysqli_query($mycon1,"SELECT `amount` FROM `collection` WHERE `acctno`='$caseno' AND `description`='".$prcafetch['productdesc']."' AND `accttitle` LIKE '%AR%%PF%'");
...
$zxamount=0;
$zxsql=mysqli_query($mycon1,"SELECT `amount` FROM `collection` WHERE `acctno`='$caseno' AND `description`='$prdoctor' AND `accttitle` LIKE '%AR%%PF%'");
...
$logall2="$prdoctor<->".$pragr."<->".$praad."<->".($prahm+$zxamount)."<->".$prap1."<->".$prap2."<->".($praex-$zxamount)."<->|";
...
echo "
        <tr>
          ...
          <td><div align='right' class='times10black'>&nbsp;".number_format(($prahm+$zxamount),2,'.',',')."&nbsp;</div></td>
          ...
          <td><div align='right' class='times10black'>&nbsp;".number_format(($praex-$zxamount),2,'.',',')."&nbsp;</div></td>
        </tr>
";
}
...
$lessacct=mysql_query("SELECT type as status,accttitle as acctitle, amount, datearray,refno,ofr FROM collection where acctno='$caseno'  and accttitle like '%AR %'");
...
echo "
          <tr>
            <td align='center' class='times10black'><table border='0' width='100%'>
              <tr>
                <td width='30%'>&nbsp; </td>
