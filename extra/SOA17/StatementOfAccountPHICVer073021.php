...
echo "
        ...
        ";
                       ...
                       // $lessacct=mysql_query("SELECT SUBSTRING_INDEX(type, ' ', 2) as status,SUBSTRING_INDEX(accttitle, ' ', 2) as acctitle,sum(amount) as amount,datearray,refno,ofr FROM collection where acctno='$caseno' and accttitle like '%AR %' ");
                      //$lessacct=mysql_query("SELECT type as status,accttitle as acctitle,amount as amount,datearray,refno,ofr FROM collection where acctno='$caseno' and accttitle like '%AR %' ");
                   $lessacct=mysql_query("SELECT type as status,accttitle as acctitle,sum(amount) as amount,datearray,refno,ofr FROM collection where acctno='$caseno'  and accttitle like '%AR %'   group by SUBSTRING_INDEX(accttitle, ' ', 2)");
                    ...
                    while($lessacctfetch=mysql_fetch_array($lessacct))
                       ...
                       // $lessacct=mysql_query("SELECT * FROM acctgenledge where caseno='$caseno'");
                        $lesscollect=mysql_query("SELECT * FROM collection where acctno='$caseno' and (accttitle='PATIENTS DEPOSIT' OR accttitle='PF DEPOSIT' OR accttitle like '%AR TRADE%') and type !='pending' ");
...
//start-- sum of the total Less (AR and Patient's Deposit)
          $total_AR=0;
          $AR_query=mysql_query("SELECT * FROM acctgenledge where caseno='$caseno' and (acctitle like '%AR%' OR acctitle like '% PF%' OR acctitle not like '%AR TRADE%')  ");
