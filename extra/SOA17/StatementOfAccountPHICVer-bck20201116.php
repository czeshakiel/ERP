                  ...
                  <td width='auto' class='table1Bottom' valign='middle'><a href='AddDetails.php?caseno=$caseno&type=1' class='astyle' target='_blank'><div align='left'>
                    <table border='0' width='100%' cellpadding='0' cellspacing='0'>
                      <tr>
                        ...
                        <td width='auto'><div align='left' class='times8black2'>$c1disp&nbsp;</div></td>
                      <tr>
                    </table>
                  </div></a></td>
                </tr>
...
$pfadphic12=0;
$pfadexcess2=0;
$pfadsql2=mysql_query("SELECT productcode, sellingprice, quantity, adjustment, gross, hmo, phic, phic1, excess FROM productout WHERE caseno='$caseno' AND trantype='charge' AND (producttype LIKE 'IPD admitting' OR producttype LIKE 'ADMITTING' OR producttype LIKE 'IPD discharge') AND productsubtype='PROFESSIONAL FEE'");
...
//$res= fopen("Logs/$caseno-PFDetails.txt", "w") or die("Unable to open file!");
//$res2= fopen("Logs/$caseno-PF.txt", "w") or die("Unable to open file!");
...
//COMANAGE
$asdfsql=mysql_query("SELECT sellingprice, quantity, adjustment, gross, phic, hmo, producttype, phic, phic1, excess FROM productout WHERE caseno='$caseno' AND trantype='charge' AND producttype LIKE 'IPD comanaged' AND productsubtype='PROFESSIONAL FEE'");
while($asdffetch=mysql_fetch_array($asdfsql)){
$pfactual22+=$asdffetch['sellingprice']*$asdffetch['quantity'];
$pfadj22+=$asdffetch['adjustment'];
$pfhmo22+=$asdffetch['hmo'];
$pfphic22+=$asdffetch['phic'];
$pfphic122+=$asdffetch['phic1'];
$pfexcess22+=$asdffetch['excess'];
}

$pftotactual+=$pfactual22;
$pfadjsubtot+=$pfadj22;
$pfhmotot+=$pfhmo22;
$pfphictot+=$pfphic22;
$pfphic1tot+=$pfphic122;
$pfexcesstot+=$pfexcess22;
//--------

$pfactualrel=$pfactual+$pfactual2+$pfactual22-$totopfactual;
$pfadjrel=$pfadj+$pfadj2+$pfadj22-$totopfadj;
$pfhmorel=$pfhmo+$pfhmo2+$pfhmo22-$totopfhmo;
$pfphicrel=$pfphic+$pfphic2+$pfphic22-$totopfphic;
$pfphic1rel=$pfphic1+$pfphic12+$pfphic122-$totopfphic1;
$pfexcessrel=$pfexcess+$pfexcess2+$pfexcess22-$totopfexcess;
...
if($b==1){$actualplus=$pfadactual;$adjplus=$pfadadj;$grossplus=$pfadgross;$hmoplus=$pfadhmo;$phicplus=$pfadphic;$phic1plus=$pfadphic1;$excessplus=$pfadexcess;}
else{$actualplus=0;$adjplus=0;$grossplus=0;$hmoplus=0;$phicplus=0;$phic1plus=0;$excessplus=0;}

//$log="$doctor<->".($pfadjrelnew+$adjplus)."<->".($pfadjrelnew+$adjplus)."<->".($pfhmorelnew+$hmoplus)."<->".($pfphicrel+$phicplus)."<->".($pfphic1rel+$phic1plus)."<->".($pfexcessrel+$excessplus)."|";
//fwrite($res, $log);

$logall2="$doctor<->".($pfactualrel+$actualplus)."<->".($pfadjrelnew+$adjplus)."<->".($pfhmorelnew+$hmoplus)."<->".($pfphicrel+$phicplus)."<->".($pfphic1rel+$phic1plus)."<->".($pfexcessrel+$excessplus)."<->|";
...
$overap1=(($totactual-$adjsubtot-($agamount+$hmotot+$hdamt+$hd2amt+$hd3amt+$hd4amt))-$fchshare)-$sechshare;
...
//$log2=(($pftotactual-$totopfactual)+$pfadactual)."|".(($pfadjsubtot-$totopfadj)+$pfadadj)."|".(($pfhmotot-$totopfhmo)+$pfadhmo)."|".($pfphictot+$pfadphic)."|".($pfphic1tot+$pfadphic1)."|".$pftot."|";
//fwrite($res2, $log2);
//fclose($res2);

$logall4=(($pftotactual-$totopfactual)+$pfadactual)."|".(($pfadjsubtot-$totopfadj)+$pfadadj)."|".(($pfhmotot-$totopfhmo)+$pfadhmo)."|".($pfphictot+$pfadphic)."|".($pfphic1tot+$pfadphic1)."|".$pftot."|<>";
...
echo "
        ...
        <tr>
          ...
          <td><div align='right' class='times13blackbold'>&nbsp;".number_format(($hmotot+$pfhmotot+$agamount+$hdamt+$hd2amt+$hd3amt+$hd4amt),2,'.',',')."&nbsp;</div></td>
