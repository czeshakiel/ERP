...
<script type="text/JavaScript">
  function showResult() {
    ...
    xmlhttp.open("GET","consolsr.php?view=ft&xix='.$xix.'&yiy='.$yiy.'&show='.$show.'&page='.$page.'&searchme="+document.searchme.searchme.value,true);
    xmlhttp.send();
  }
</script>
';
    ...
    <tr>
      <td><table border='0' width='100%' cellpadding='0' cellspacing='0'>
        <tr>
          ...
          <form name='searchme' method='post'>
...
$pagesql=mysqli_query($conn,"SELECT `caseno`, `patid`, `dateadmit`, `rducaseno` FROM `rduconsolidate` WHERE `dateadmit` IN (SELECT MAX(`dateadmit`) FROM `rduconsolidate` GROUP BY `rducaseno`)  AND `remarks` NOT LIKE 'Transmitted' ORDER BY `dateadmit`");
