...
$a=0;
$asql=mysqli_query($conn,"SELECT `caseno`, `patid`, `dateadmit`, `patientname`, `rducaseno` FROM `rduconsolidate` WHERE `patientname` LIKE '%$searchme%' AND `remarks` NOT LIKE 'Transmitted' GROUP BY `rducaseno` ORDER BY `dateadmit`");
...
$a=$page;
$asql=mysqli_query($conn,"SELECT `caseno`, `patid`, `patientname`, `dateadmit`, `rducaseno`, `remarks` FROM `rduconsolidate` WHERE `dateadmit` IN (SELECT MAX(`dateadmit`) FROM `rduconsolidate` GROUP BY `rducaseno`) AND `remarks` NOT LIKE 'Transmitted' ORDER BY `dateadmit` LIMIT $page,$show");
while($afetch=mysqli_fetch_array($asql)){
$caseno=$afetch['caseno'];
$rducaseno=$afetch['rducaseno'];
$patientidno=$afetch['patid'];
...
$pagesql=mysqli_query($conn,"SELECT `caseno`, `patid`, `patientname`, `dateadmit`, `rducaseno`, `remarks` FROM `rduconsolidate` WHERE `dateadmit` IN (SELECT MAX(`dateadmit`) FROM `rduconsolidate` GROUP BY `rducaseno`) AND `remarks` NOT LIKE 'Transmitted'");
