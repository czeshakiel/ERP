...
$hthmo=0;
$alloth="";
$stsql=mysqli_query($conn,"SELECT `autono`, `label` FROM `soasetup` WHERE `label`='$label'");
...
";
  ...
  while($sdfetch=mysqli_fetch_array($sdsql)){
    ...
    if($type==1){
      ...
      $posql=mysqli_query($conn,"SELECT `productcode`, `productdesc`, `productsubtype`, cast(`sellingprice` as decimal(10,2)) AS `sp`, cast(`quantity` as decimal(10,2)) AS `qty`, cast(`adjustment` as decimal(10,2)) AS `adj`, cast(`phic` as decimal(10,2)) AS `ph1`, cast(`phic1` as decimal(10,2)) AS `ph2`, cast(`hmo` as decimal(10,2)) AS `hmo`, cast(`excess` as decimal(10,2)) AS `exc` FROM `productout` WHERE `caseno`='$caseno' AND `trantype`='charge' AND `quantity` > 0 AND `productsubtype` LIKE '$productsubtype' AND `producttype` LIKE '$producttype'");
    }
    //---------------------------------
...
//END ALL OTHERS---------------------------------------------------------------------------------------------------------------------------
    ...
    $xysql=mysqli_query($conn,"SELECT `patientidno` FROM `admission` WHERE `caseno`='$caseno'");
    $xyfetch=mysqli_fetch_array($xysql);
    $patid=$xyfetch['patientidno'];

    $xzsql=mysqli_query($conn,"SELECT `patientname` FROM `patientprofile` WHERE `patientidno`='$patid'");
    ...
    if($xzcount!=0){
      ...
      $xxsql=mysqli_query($conn,"SELECT `name` FROM `nsauthemployees` WHERE `empid`='$patid'");
      $xxfetch=mysqli_fetch_array($xxsql);
      $patname=strtoupper($xxfetch['name']);
    }

    $allothpf="";
    $allotpfsql=mysqli_query($conn,"SELECT `producttype` FROM `soasetupdetails` WHERE `productsubtype`='PROFESSIONAL FEE' GROUP BY `producttype`");
...
";
    ...
    $z=0;
    $zsql=mysqli_query($conn,"SELECT `accttitle` FROM `collection` WHERE `acctno`='$caseno' AND `accttitle` LIKE '%AR%%PF%' GROUP BY `accttitle`");
...
echo "
    <tr>
      ...
      <td class='b1 l1' height='20'><div align='left' class='courier s12 black' title='$apttyp'>&nbsp;$apdesc&nbsp;</div></td>
...
";
      ...
      for($y=1;$y<=$z;$y++){
        $yacc[$y]=$zstore[$y];
        $ysql=mysqli_query($conn,"SELECT SUM(`amount`) AS `amt` FROM `collection` WHERE `acctno`='$caseno' AND `description`='$apdesc' AND `accttitle` LIKE '$yacc[$y]'");
