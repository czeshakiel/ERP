...
$ttime=mysqli_real_escape_string($conn,$_POST['ttime']);
...
$tdssql=mysqli_query($conn,"SELECT * FROM `tempdatestorage` WHERE `caseno`='$caseno'");
...
if($tdscount!=0){
  ...
  mysqli_query($conn,"UPDATE `tempdatestorage` SET `date`='$tdate', `time`='$ttime' WHERE `caseno`='$caseno'");
}
else{
  ...
  mysqli_query($conn,"INSERT INTO `tempdatestorage` (`caseno`, `date`, `time`) VALUES ('$caseno', '$tdate', '$ttime')");
}
