...
";
  ...
  if(isset($_GET['manual'])){
    ...
    $a=0;
    $asql=mysqli_query($mycon1,"SELECT r.unit, r.pnf, r.lotno, r.testcode, r.gtestcode, r.code, r.description, s.generic, SUM(s.quantity) AS soh FROM receiving r, stocktable s WHERE r.code=s.code AND (r.unit LIKE '%PHARMACY/SUPPLIES%' OR r.unit LIKE '%MEDICAL SURGICAL SUPPLIES%') AND s.description LIKE '%$searchme%' AND s.dept='$stk' GROUP BY r.code");
    //$asql=mysqli_query($mycon1,"SELECT r.unit, r.pnf, r.lotno, r.testcode, r.gtestcode, r.code, r.description, r.generic FROM receiving r WHERE (r.unit LIKE '%PHARMACY/SUPPLIES%' OR r.unit LIKE '%MEDICAL SURGICAL SUPPLIES%') AND r.description LIKE '%$searchme%'");
