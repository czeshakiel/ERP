...
$rchksql=mysqli_query($mycon1,"SELECT trantype,administration FROM productout WHERE refno='$refno'");
...
if($tt=="PAID"){
  ...
  echo "<META HTTP-EQUIV='Refresh'CONTENT='3;URL=http://$setip/2021codes/ChargeCart/ccpharmacy_manual.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
}
else{
  if($sta=='pending'){
    $da=date("YmdHis");
    ...
    echo "<META HTTP-EQUIV='Refresh'CONTENT='1;URL=http://$setip/2021codes/ChargeCart/ccpharmacy_manual.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
  }
  else{
...
";
  ...
  if(isset($_GET['manual'])){
    ...
    $a=0;
    $asql=mysqli_query($mycon1,"SELECT r.unit, r.pnf, r.lotno, r.testcode, r.gtestcode, s.code, s.description, s.generic, SUM(s.quantity) AS soh FROM stocktable s, receiving r WHERE s.code=r.code AND r.unit='PHARMACY/MEDICINE' AND (r.description LIKE '%$searchme%' OR r.generic LIKE '$searchme%') AND s.dept='$stk' GROUP BY s.code");
