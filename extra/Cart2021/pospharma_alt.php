...
if($toh=="PHARMACY_QUOT"){
  $toaddress="ccpharmacy_quot.php";
  $dispense="off";
}
else if($toh=="CSR2_QUOT"){
  $toaddress="cccsr2_quot.php";
  $dispense="off";
}
...
$ptcsql=mysql_query("SELECT policyno FROM admission WHERE caseno='$caseno'");
$ptcfetch=mysql_fetch_array($ptcsql);
...
$ppsql=mysql_query("SELECT senior FROM quotation WHERE caseno='$caseno'");
...
$stuc=mysql_query("SELECT unitcost FROM stocktable WHERE code='$code' AND trantype NOT LIKE 'dispensed' AND trantype NOT LIKE 'return' AND unitcost > 0 ORDER BY datearray");
...
//    echo "INSERT INTO `productout` (`refno`, `invno`, `caseno`, `productcode`, `productdesc`, `sellingprice`, `quantity`, `adjustment`, `gross`, `trantype`, `phic`, `hmo`, `excess`, `date`, `status`, `terminalname`, `loginuser`, `batchno`, `producttype`, `productsubtype`, `approvalno`, `referenceno`, `administration`, `shift`, `location`, `datearray`, `phic1`) VALUES ('$refno', '$invno', '$caseno', '$code', '$descrel', '$price', '$relqty', '$adjustment', '$gross', '$trantype', '$phic1', '$hmo', '$excess', '$date', '$status', '$rrno', '$user', '$batchno', '$tagging', '$unit', '', '', 'pending', 'MMSHI', '', '$pdate', '$phic2')<br />";
    ...
    mysqli_query($mycon1,"INSERT INTO `quotationdetails` (`caseno`, `productcode`, `productdesc`, `quantity`, `price_cash`, `cash_gross`, `cash_disc`, `price_charge`, `datearray`, `nursename`) VALUES ('$caseno', '$code', '$descrel', '$relqty', '$spcash', '$cag', '$cad', '$spcharge', '".date("Y-m-d")."', '$user')");
