...
if(strpos($caseno, "WPOS") !== false){
  $pasql=mysqli_query($conn,"SELECT `remarks` AS `senior` FROM `admission` WHERE `caseno`='$caseno'");
  $ppsql=mysqli_query($conn,"SELECT `patientname` FROM `patientprofile` WHERE `patientidno`='$patientidno'");
  $pafetch=mysqli_fetch_array($pasql);
  $senior=$pafetch['senior'];
...
if($toh=="PACKAGE"){$kstqty="100";}echo $kstqty;
...
if($relqty==0){
echo "<META HTTP-EQUIV='Refresh'CONTENT='0;URL=../Cart2021/$toaddress?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";break;
}
//
}
...
echo "<META HTTP-EQUIV='Refresh'CONTENT='5;URL=../Cart2021/$toaddress?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
}
}
