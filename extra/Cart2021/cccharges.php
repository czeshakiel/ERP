...
$patsql=mysqli_query($mycon1,"SELECT p.patientname, a.patientidno FROM admission a, patientprofile p WHERE a.caseno='$caseno' AND a.patientidno=p.patientidno");
...
$rchksql=mysqli_query($mycon1,"SELECT productcode, terminalname, status, trantype, productsubtype, approvalno FROM productout WHERE refno='$refno'");
...
$miscsql=mysqli_query($mycon1,"SELECT `SEMIPRIVATE` FROM `receiving` WHERE `code`='$pco' AND `PRIVATE`='misc'");
$misccount=mysqli_num_rows($miscsql);

$miscaddsql=mysqli_query($mycon1,"SELECT `code` FROM `receiving` WHERE `SEMIPRIVATE`='$pco' AND `PRIVATE`='misc'");
    ...
    if($ptr=="cash"){
      if($stt=="PAID"){
        ...
        if($misccount!=0){
          ...
          mysqli_query($mycon1,"SELECT * FROM productout WHERE productcode='$misccd' AND approvalno='PA-$refno' INTO OUTFILE '/opt/lampp/htdocs/2021codes/ChargeCart/DelLog/MISC-$refno-$da-$ccname.txt' FIELDS TERMINATED BY '|'");
          mysqli_query($mycon1,"DELETE FROM productout WHERE productcode='$misccd' AND approvalno='PA-$refno'");
        }
        ...
        if($miscaddcount!=0){
          ...
          mysqli_query($mycon1,"SELECT * FROM productout WHERE productcode='$miscaddcd' AND refno='$appref' INTO OUTFILE '/opt/lampp/htdocs/2021codes/ChargeCart/DelLog/MISCI-$refno-$da-$ccname.txt' FIELDS TERMINATED BY '|'");
          mysqli_query($mycon1,"DELETE FROM productout WHERE productcode='$miscaddcd' AND refno='$appref'");
          mysqli_query($mycon1,"DELETE FROM labpending WHERE refno='$appref'");
        }
        ...
        if($pty=="LABORATORY"){
          mysqli_query($mycon1,"DELETE FROM labpending WHERE refno='$refno'");
        }
...
$a=0;
$presdate=date("Y-m-d");
$asql=mysqli_query($mycon1,"SELECT refno, productcode, productdesc, quantity, trantype FROM productout WHERE caseno='$caseno' AND batchno='$tick'");
...
$a++;
$zsql=mysqli_query($mycon1,"SELECT po.`refno`, a.`caseno` FROM `admission` a, `productout` po WHERE a.`patientidno`='$patientidno' AND a.`caseno` NOT LIKE '$caseno' AND a.`dateadmit`='$presdate' AND a.`caseno`=po.`caseno` AND po.`productcode`='$ccode' AND (po.`trantype`='cash' OR po.`trantype`='charge') AND (po.`status`='PAID' OR po.`status`='Approved')");
$zcount=mysqli_num_rows($zsql);

$ysql=mysqli_query($mycon1,"SELECT `remarks` FROM `labtest` WHERE `refno`='$refno'");
