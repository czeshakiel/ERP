...
$rchksql=mysqli_query($mycon1,"SELECT productcode, terminalname, status, trantype, productsubtype FROM productout WHERE refno='$refno'");
...
$zsql=mysqli_query($mycon1,"SELECT po.`refno` FROM `admission` a, `productout` po WHERE a.`patientidno`='$patientidno' AND a.`caseno` NOT LIKE '$caseno' AND a.`dateadmit`='$presdate' AND po.`productcode`='$ccode' AND po.`trantype`='cash' AND po.`status`='PAID'");
