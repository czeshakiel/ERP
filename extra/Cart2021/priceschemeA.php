...
//PRICE ADJUSTMENT 2022-06-22 UPDATED--------------------------------------------------------------
        ...
        $sellingprice20230105=$charger+$chargeadd;
//-------------------------------------------------------------------------------------------------

//PRICE ADJUSTMENT 2023-01-05 UPDATED BY MARK------------------------------------------------------
        $charge4=($sellingprice20230105+($sellingprice20230105*0.05));
        $charger=number_format($charge4,2);
        $charger=str_replace(",","",$charger);
        $chargestr=substr($charger, -1);
...
//-------------------------------------
    ...
    if($toh=="PACKAGE"){
      if((isset($_POST['sprel']))&&(isset($_POST['itd']))){
        $sellingprice=mysqli_real_escape_string($conn,$_POST['sprel']);
        $adj=mysqli_real_escape_string($conn,$_POST['itd']);
      }
      else{
        $sellingprice=mysqli_real_escape_string($conn,$_GET['sprel']);
        $adj=mysqli_real_escape_string($conn,$_GET['itd']);
      }
    }
