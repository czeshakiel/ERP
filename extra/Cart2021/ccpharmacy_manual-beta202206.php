...
echo "SELECT r.unit, r.pnf, r.lotno, r.testcode, r.gtestcode, s.code, s.description, s.generic, SUM(s.quantity) AS soh FROM receiving r, stocktable s WHERE r.code=s.code AND r.unit='PHARMACY/MEDICINE' AND (s.description LIKE '%$searchme%' OR s.generic LIKE '$searchme%') AND s.dept='$stk' GROUP BY s.code, s.dept";
    $a=0;
    $asql=mysqli_query($mycon1,"SELECT s.code, s.description, s.generic FROM receiving r, stocktable s WHERE r.code=s.code AND r.unit='PHARMACY/MEDICINE' AND (s.description LIKE '%$searchme%' OR s.generic LIKE '$searchme%') AND s.dept='$stk' GROUP BY s.code");
    ...
    echo "
      <form method='post' action='pospharma.php'>
        <tr $bg>
          <td height='35'><div align='center' class='arial s14 black'>&nbsp;$a -- $co&nbsp;</div></td>
