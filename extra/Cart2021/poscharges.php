...
if($toh=="PACKAGE"){
  ...
  //container----------------------------------------------------------------------------------------
  $kconsql=mysqli_query($conn,"SELECT `description`, `PRIVATE`, `SEMIPRIVATE`, `testcode` FROM `receiving` WHERE `code`='$code'");
  ...
  if($trantype=="cash"){
    ...
    if(stripos($itmdsc, "-SENDOUT") !== FALSE){
...
//END PACKAGE----------------------------------------------------------------------------------------------------------------------------------------
  ...
  if($trantype=="cash"){
    ...
    echo "<META HTTP-EQUIV='Refresh'CONTENT='0;URL=../Cart2021/$retadd?caseno=$caseno&station=$station&toh=$toh&tick=$tick'>";
  }
  else if($trantype=="charge"){
  //KNOW CREDIT LEFT-----------------------------
