...
setcookie("ccname", $ccname, time() + 300, "/");
  ...
  $csql=mysqli_query($mycon1,"SELECT `patientidno`, `membership`, `hmomembership`, `policyno`, `status`, `addemployer` FROM `admission` WHERE `caseno`='$caseno'");
  ...
  if($trantype=="cash"){
    ...
    if($addemployer!=''){
      ...
      $cslsql=mysqli_query($mycon1,"SELECT `price` FROM `comsplist` WHERE `code`='$code' AND `company`='$addemployer'");
...
//container----------------------------------------------------------------------------------------
$kconsql=mysqli_query($mycon1,"SELECT `PRIVATE`, `SEMIPRIVATE`, `testcode` FROM `receiving` WHERE `code`='$code'");
