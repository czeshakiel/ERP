<?php
      ...
      $zxsql=mysqli_query($conn,"SELECT r.`unit`, r.`pnf`, r.`lotno`, r.`testcode`, r.`gtestcode`, r.`code`, r.`description`, r.`generic`, SUM(s.`quantity`) AS `soh`, r.`optset4`, r.`itemname`, r.`lotno` FROM `stocktable` s, `receiving` r WHERE s.`code`=r.`code` AND (r.`unit` LIKE 'PHARMACY/SUPPLIES' OR r.`unit` LIKE 'MEDICAL SURGICAL SUPPLIES' OR r.`unit` LIKE 'MEDICAL SUPPLIES') AND (r.`description` LIKE '%$searchme%' OR r.`generic` LIKE '$searchme%') AND s.`dept`='$dept' $ptq GROUP BY s.`code` ORDER BY r.`description`, r.`generic` ASC");
...
echo "
                                  <button type='button' name='edt' class='btn' title='Edit'"; ?> onclick="<?php echo "window.open('ms/?code=$cod".$usr."', '_blank', 'toolbar=yes,scrollbars=yes,resizable=yes,top=30,left=700,width=480,height=810');";?>" <?php echo ">&#x270E;</button>
";
          }
...
echo "
                              <tr>
                                <td class='t2' colspan='$alwvcol'><div align='left' style='font-family: arial;font-size: 11px;padding: 3px 5px;'>
