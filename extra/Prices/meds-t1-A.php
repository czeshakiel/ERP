<?php
      ...
      if($pt=="M"){$ptq="AND r.`lotno` NOT LIKE 'S'";$mckb="disabled";$prcds="prcdis";}
      else if($pt=="S"){$ptq="AND r.`lotno`='S'";$mckb="";$prcds="prc";}
      else{$ptq="";$mckb="";$prcds="prc";}

      if(isset($_GET['sdpt'])){
        if($_GET['sdpt']=="PHARMACY"){$alwv=0;$alwvcol=10;}
        else{$alwv=0;$alwvcol=10;}
      }
      else{$alwv=1;$alwvcol=11;}

      $zxsql=mysqli_query($conn,"SELECT r.`unit`, r.`pnf`, r.`lotno`, r.`testcode`, r.`gtestcode`, r.`code`, r.`description`, r.`generic`, SUM(s.`quantity`) AS `soh`, r.`optset4`, r.`itemname`, r.`lotno` FROM `stocktable` s, `receiving` r WHERE s.`code`=r.`code` AND r.`unit`='PHARMACY/MEDICINE' AND (r.`description` LIKE '%$searchme%' OR r.`generic` LIKE '$searchme%') AND s.`dept`='$dept' $ptq GROUP BY s.`code` ORDER BY r.`description`, r.`generic` ASC");
...
echo "
                            <table border='0' cellpadding='0' cellspacing='0' class='hoverTable'>
                              <tr>
                                ...
                                <td class='t2 b2 l1' style='border-radius: 0px;'><div align='center' style='font-family: arial;font-size: 11px;font-weight: bold;color: #000000;padding: 3px 5px;'><input type='checkbox' class='checkall' id='select_all' $mckb /></div></td>
...
echo "
                              <tr>
                                ...
                                <td class='b1 l1' style='border-radius: 0px;'><div align='center' style='font-family: arial;font-size: 14px;color: #000000;padding: 3px 5px;'><input type='checkbox' class='case' name='code[]' value='$cod' $ckb /></div></td>
                                <td class='b1 l1' style='border-radius: 0px;'><div align='left' style='font-family: arial;font-size: 14px;color: #000000;padding: 3px 5px;' title='$tes'>$itd"; if($itg!=""){echo " [<span style='color: blue;font-size: 10px;'>".$itg."</span>]";} echo "</div></td>
...
echo "
                                  <button type='button' type='button' name='edt' class='btn' title='Edit'"; ?> onclick="<?php echo "window.open('ms/?code=$cod".$usr."', '_blank', 'toolbar=yes,scrollbars=yes,resizable=yes,top=30,left=700,width=480,height=810');";?>" <?php echo ">&#x270E;</button>
";
          }
...
echo "
                              <tr>
                                <td class='t2' colspan='$alwvcol'><div align='center' style='padding: 3px 5px;'>
                                  <button type='submit' name='prc' class='$prcds' $mckb>Change Price</button>
                                <div></td>
                              </tr>
";
        }
...
echo "
                              <tr>
                                <td colspan='$alwvcol'><div align='left' style='font-family: arial;font-size: 11px;padding: 3px 5px;'>
