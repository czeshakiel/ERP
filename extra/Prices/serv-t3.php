                                ...
                                <table border='0' cellpadding='0' cellspacing='0'>
                                  ...
                                  <tr>
                                    <td><div align='center' style='padding: 10px 0;font-family: arial;color: #000000;'>
                                      <span style='font-size: 15px;font-weight: bold;'>$unit SERVICES PRICE LIST</span><br />
...
";
  ...
  if($unit=="LABORATORY"){
echo "
                                          <td bgcolor='3380ff' width='auto' class='t2 b2 l1' rowspan='2'><div align='center' class='arial s12 white bold' style='padding: 2px 3px;'>Test Type</div></td>
";
  }
...
";
    ...
    $colsp=6;
    if($unit==="LABORATORY"){
      if($lotno=="others"){$zxsql=mysqli_query($conn,"SELECT * FROM `receiving` WHERE `unit` LIKE '$unit' AND `lotno` NOT LIKE 'hematology' AND `lotno` NOT LIKE 'chemistry' AND `lotno` NOT LIKE 'serology' AND `lotno` NOT LIKE 'clinical microscopy' AND `lotno` NOT LIKE 'parasitology' ORDER BY `unit`, `description`");}
      else{$zxsql=mysqli_query($conn,"SELECT * FROM `receiving` WHERE `unit` LIKE '$unit' AND `lotno` LIKE '%$lotno%' ORDER BY `unit`, `description`");}
    }
    else{
      $zxsql=mysqli_query($conn,"SELECT * FROM `receiving` WHERE `unit` LIKE '$unit' ORDER BY `unit`, `description`");
    }
...
";
      ...
      if($unit=="LABORATORY"){
        $colsp=7;
echo "
                                          <td class='b1 l1'><div align='center' style='font-family: arial;font-size; 13px;color: #000000;padding: 2px 4px;'>$testcat</div></td>
";
      }
