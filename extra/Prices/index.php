...
<style>
  ...
  .num{border: 2px solid #000000;border-radius: 5px;height: 23px;font-family: arial;font-size: 12px;font-weight: bold;padding: 2px 5px;}
  .num:hover{background-color: #DAF7A6;}
  .num:focus{background-color: #DAF7A6;}
  .prc{height: 30px;font-size: 14px;font-weight: bold;background-color: #FFC300;color: #000000;border: 1px solid #000000;border-radius: 3px;padding: 0 10px;}
  .prc:hover{background-color: #F6A531;color: #FFFFFF;}
  .prcdis{height: 30px;font-size: 14px;font-weight: bold;background-color: #DBDBDB ;color: #000000;border: 1px solid #000000;border-radius: 3px;padding: 0 10px;}
  .prv{height: 30px;font-size: 14px;font-weight: bold;background-color: #FA3D54;color: #FFFFFF;border: 1px solid #000000;border-radius: 3px;padding: 0 10px;}
  .prv:hover{background-color: #F78391;color: #000000;}
</style>
</head>
<?php
...
if(isset($_GET['xox'])){
  ...
  if(isset($_GET['sdpt'])){
    $sdpt=mysqli_real_escape_string($conn,$_GET['sdpt']);
    $usr="&xox=".$user."&sdpt=".$sdpt;
  }

  $zasql=mysqli_query($conn,"SELECT * FROM `nsauth` WHERE `station`='Masterfile' AND `username`='".base64_decode($user)."'");
  if(mysqli_num_rows($zasql)>0){$up=1;$inv=1;$onv=1;}

  $ezsql=mysqli_query($conn,"SELECT *  FROM `nsauth` WHERE `station`='PHARMACY' AND `username`='".base64_decode($user)."' AND `Access`='5'");
  if(mysqli_num_rows($ezsql)>0){$up=1;$inv=1;}

  $fzsql=mysqli_query($conn,"SELECT *  FROM `nsauth` WHERE `station`='CPU' AND `username`='".base64_decode($user)."' AND `Access`='5'");
  if(mysqli_num_rows($fzsql)>0){$up=1;$inv=1;}
}
else{
  $usr="";
}
...
if(isset($_GET['xix'])){
  $usr=$usr."&xix=".$_GET['xix'];
  if(base64_decode($_GET['xix'])=="CPU"){
    $onv=0;
  }
}
...
if(isset($_GET['meds'])){
  ...
  if(isset($_GET['sdpt'])){
    if($_GET['sdpt']=="PHARMACY"){$othmenu=0;$msmenu=1;}
    else{$othmenu=0;$msmenu=0;}
  }
  else{$othmenu=1;$msmenu=1;}
