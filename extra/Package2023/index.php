...
if(isset($_POST['caseno'])){$caseno=mysqli_real_escape_string($conn,$_POST['caseno']);}else{$caseno=mysqli_real_escape_string($conn,$_GET['caseno']);}
if(isset($_POST['dept'])){$dept=mysqli_real_escape_string($conn,$_POST['dept']);}else{$dept=mysqli_real_escape_string($conn,$_GET['dept']);}
if(isset($_POST['ticket'])){$ticket=mysqli_real_escape_string($conn,$_POST['ticket']);}else{$ticket=mysqli_real_escape_string($conn,$_GET['ticket']);}
if(isset($_POST['station'])){$station=mysqli_real_escape_string($conn,$_POST['station']);}else{$station=mysqli_real_escape_string($conn,$_GET['station']);}
...
if(isset($_POST['user'])){$frm=1;$nursename=mysqli_real_escape_string($conn,$_POST['user']);}else{$frm=2;$nursename=$ccname;}
if(isset($_POST['enterpassword'])){$ipass=mysqli_real_escape_string($conn,$_POST['enterpassword']);}else{$ipass=$ccpass;}

$asql=mysqli_query($conn,"SELECT * FROM `nsauth` WHERE `password`='$ipass'");
...
if($station!="RDU"){
$bsql=mysqli_query($conn,"SELECT * FROM `packagelist` WHERE `dept` LIKE '' ORDER BY `packagename`");
    ...
    <tr>
      ...
      <td class='b1 l1 r2' height='40'><div align='center' style='font-family: arial;color: #000000;font-size: 15px;padding: 2px 5px 2px 5px;'>
        <form name='View' method='post' action='vp.php'>
          <input type='submit' name='View' style='background-color: #FF5733;color: #FFFFFF;font-weight: bold;border: 1px solid black;padding: 3px 8px 3px 8px;border-radius: 4px;' title='Add Package' value='  +  ' />
...
$bsql=mysqli_query($conn,"SELECT * FROM `packagelist` WHERE `dept`='RDU' and `pckgno` NOT LIKE 'PCKG-20210222035829' ORDER BY `packagename`");
while($bfetch=mysqli_fetch_array($bsql)){
$pno=$bfetch['pckgno'];
$pn=$bfetch['packagename'];
$pr=$bfetch['price'];
