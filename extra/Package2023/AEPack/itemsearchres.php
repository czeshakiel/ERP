...
$dsql=mysql_query("SELECT receiving.code, receiving.description, productsmasterlist.opd FROM receiving, productsmasterlist WHERE receiving.code=productsmasterlist.code AND receiving.description LIKE '%$searchme%' AND (receiving.unit='LABORATORY' OR receiving.unit='RDU SUPPLIES' OR receiving.unit='RDU-SUPPLIES' OR receiving.unit='PROFESSIONAL FEE' OR receiving.unit='ROOM ACCOMODATION' OR receiving.unit='MEDICAL EQUIPMENT' OR receiving.unit='NURSING SERVICE FEE' OR receiving.unit='HEARTSTATION' OR receiving.unit='ECG' OR receiving.unit='ULTRASOUND' OR receiving.unit='MAMMOGRAPHY' OR receiving.unit='XRAY' OR receiving.unit='MISCELLANEOUS' OR receiving.unit='PROFESSIONAL FEE' OR receiving.unit='PHARMACY/SUPPLIES')");
...
echo "
  <tr>
    ...
    <td class='b1 l1 r2' style='padding: 5px;'><div align='center'><button class='arial s12 white bold bggreen borderblack' style='height: 30px;width: 30px;' ";?> onclick="<?php echo "window.open('setprice.php?pckgno=$pckgno&code=$dcode&desc=$desc&price=$OPD&user=$user', '_blank', 'toolbar=yes,scrollbars=yes,resizable=yes,top=$hei,left=$wid,width=$setw,height=$seth');"; ?>" <?php echo ">+</button></div></td>
  </tr>
";
}
