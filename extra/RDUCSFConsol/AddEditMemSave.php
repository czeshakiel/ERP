...
//claiminfo
$asql=mysqli_query($conn,"SELECT * FROM `claiminfo` WHERE `caseno`='$caseno'");
$acount=mysqli_num_rows($asql);
if($acount==0){
  mysqli_query($conn,"INSERT INTO `claiminfo` (`patientidno`, `caseno`, `lastname`, `firstname`, `middlename`, `identificationno`) VALUES ('$patientidno', '$caseno', '$lastname', '$firstname', '$middlename', '$identificationno')");
}
else{
  mysqli_query($conn,"UPDATE `claiminfo` SET `lastname`='$lastname', `firstname`='$firstname', `middlename`='$middlename', `identificationno`='$identificationno' WHERE `caseno`='$caseno'");
}

//claiminfoadd
$bsql=mysqli_query($conn,"SELECT * FROM `claiminfoadd` WHERE `caseno`='$caseno'");
...
mysqli_query($conn,"UPDATE `admission` SET `paymentmode`='$paymentmode' WHERE `caseno`='$caseno'");
