...
//-------------------------------------------------------------------------------------------------
$rdc1sql=mysqli_query($conn,"SELECT `dateadmit`, `caseno` FROM `rduconsolidate` WHERE `rducaseno`='$caseno' ORDER BY `dateadmit` ASC");
$rdc1fetch=mysqli_fetch_array($rdc1sql);
$dateadm=$rdc1fetch['dateadmit'];
$casenostr=$rdc1fetch['caseno'];

$ad1sql=mysqli_query($conn,"SELECT `timeadmitted`, `dateadmit` FROM `admission` WHERE `caseno`='$casenostr'");
$ad1fetch=mysqli_fetch_array($ad1sql);
$ad1ta=$ad1fetch['timeadmitted'];
$ad1da=$ad1fetch['dateadmit'];

$admdati=$ad1da." ".$ad1ta;
$admda=date("m-d-Y",strtotime($admdati));
$admti=date("h:i:sA",strtotime($admdati));

$rdc2sql=mysqli_query($conn,"SELECT `dateadmit`, `caseno` FROM `rduconsolidate` WHERE `rducaseno`='$caseno' ORDER BY `dateadmit` DESC");
$rdc2fetch=mysqli_fetch_array($rdc2sql);
$datedis=$rdc2fetch['dateadmit'];
$casenoend=$rdc2fetch['caseno'];

$ad2sql=mysqli_query($conn,"SELECT `timeadmitted`, `dateadmit` FROM `admission` WHERE `caseno`='$casenoend'");
$ad2fetch=mysqli_fetch_array($ad2sql);
$ad2ta=$ad2fetch['timeadmitted'];
$ad2da=$ad2fetch['dateadmit'];

$disdati=$ad2da." ".$ad2ta;
$disda=date("m-d-Y",strtotime($disdati));
$disti=date("h:i:sA",strtotime($disdati));
//-------------------------------------------------------------------------------------------------

$cursty="style='cursor: pointer;'";

//admission
$admsql=mysqli_query($conn,"SELECT `patientidno`, `type`, `paymentmode`, `room`, UPPER(`street`) AS `street`, UPPER(`barangay`) AS `barangay`, UPPER(`municipality`) AS `municipality`, UPPER(`province`) AS `province`, `zipcode`, UPPER(`pastmed`) AS `pastmed`, UPPER(`initialdiagnosis`) AS `initialdiagnosis`, UPPER(`finaldiagnosis`) AS `finaldiagnosis`, UPPER(`ap`) AS `ap`, `timeadmitted`, `dateadmit` FROM `admission` WHERE `caseno`='$casenostr'");
...
$dateadmitfmt=date("m-d-Y",strtotime($ad1da));
$datetimead=$ad1da." ".$ad1ta;
...
//Get Previous Data Start--------------------------------------------------------------------------
$rducaseno="";
$gpda=0;
$gpdasql=mysqli_query($conn,"SELECT rd.`rducaseno` FROM `admission` a, `rduconsolidate` rd  WHERE a.`patientidno`='$patientidno' AND a.`caseno`=rd.`caseno` AND rd.`remarks`='Transmitted' ORDER BY rd.`rducaseno`");
while($gpdafetch=mysqli_fetch_array($gpdasql)){
  $rducaseno=$gpdafetch['rducaseno'];
}

$cf1sql=mysqli_query($mycon3,"SELECT `pMemberPIN`, `pPEN`, `pEmployerName` FROM `cf1` WHERE `caseno`='$rducaseno'");
$cf1fetch=mysqli_fetch_array($cf1sql);
$cf1pMemberPIN=$cf1fetch['pMemberPIN'];
$cf1pPEN=$cf1fetch['pPEN'];
$cf1pEmployerName=$cf1fetch['pEmployerName'];
//Get Previous Data End----------------------------------------------------------------------------
...
if(strlen($phicofmember)==14){
  ...
  if($cf1pMemberPIN==""){
    ...
    $phicmember=str_split($cf1pMemberPIN);
...
$phicofdep=$identificationno; //Place value of PHIC Number of dependent from the DB here.
...
if(strlen($phicofdep)==14){
  ...
  $phicdep9=$phicdep[8];
  $phicdep10=$phicdep[9];
  $phicdep11=$phicdep[10];
  $phicdep13=$phicdep[11];
}
else{
  if($cf1pMemberPIN==""){
    $phicdep0="";
    ...
    $phicdep=str_split($cf1pMemberPIN);
...
$datedischarged=str_replace("_","-",$ad2da);
...
/*3. Business Name: */
if($cf1pEmployerName!=""){$empbusinessname=$cf1pEmployerName;}
