...
$rchksql=mysqli_query($conn,"SELECT administration FROM productout WHERE refno='$refno'");
...
$sta=$rchkfetch['administration'];
  if($sta=='pending'){
    ...
    echo "<META HTTP-EQUIV='Refresh'CONTENT='1;URL=http://$setip/ERP/extra/Cart/cccsr2_manual.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
  }
  else{
    echo "<span class='arial s16 red bold'>Unable to delete item. Item already dispensed.</span>";
    echo "<META HTTP-EQUIV='Refresh'CONTENT='3;URL=http://$setip/ERP/extra/Cart/cccsr2_manual.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
  }
}
else{
  //KNOW CREDIT LEFT-----------------------------
...
<div align='left'>
  <table border='0' cellpadding='0' cellspacing='0'>
    ...
    <tr>
      <td height='10'><div align='left' class='arial s14 bold'>CSR2 CART | USER: <span class='blue'>".$_COOKIE["ccname"]." $label</span></div></td>
    </tr>
";
...
";
  ...
  if(isset($_GET['manual'])){
    ...
    $a=0;
    $asql=mysqli_query($conn,"SELECT r.unit, r.pnf, r.lotno, r.testcode, r.gtestcode, r.code, r.description, s.generic, SUM(s.quantity) AS soh FROM receiving r, stocktable s WHERE r.code=s.code AND (r.unit LIKE '%PHARMACY/SUPPLIES%' OR r.unit LIKE '%MEDICAL SURGICAL SUPPLIES%') AND r.description LIKE '%$searchme%' AND s.dept='$stk' GROUP BY r.code");
    //$asql=mysqli_query($conn,"SELECT s.code, s.description, s.generic FROM receiving r, stocktable s WHERE r.code=s.code AND (r.unit LIKE '%PHARMACY/SUPPLIES%' OR r.unit LIKE '%MEDICAL SURGICAL SUPPLIES%') AND s.description LIKE '%$searchme%' AND s.dept='$stk' GROUP BY r.code");
