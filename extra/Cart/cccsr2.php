...
$sta=$rchkfetch['administration'];
  if($sta=='pending'){
    ...
    mysqli_query($conn,"SELECT * FROM productout WHERE refno='$refno' INTO OUTFILE '/opt/lampp/htdocs/2021codes/ChargeCart/DelLog/$refno-$da-$ccname.txt' FIELDS TERMINATED BY '|'");
  ...
  $losql=mysqli_query($conn,"SELECT status FROM admission WHERE caseno='$caseno'");
...
xmlhttp.open("GET","cccsr2result.php?caseno='.$caseno.'&station='.$station.'&toh='.$toh.'&tick='.$tick.'&stk='.$stk.'&at='.$totgross.'&cl='.$cl.'&searchme="+document.searchme.searchme.value,true);
xmlhttp.send();
}
