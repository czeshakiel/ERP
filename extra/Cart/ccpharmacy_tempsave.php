...
$rchksql=mysqli_query($mycon1,"SELECT administration FROM productout_tempsave WHERE refno='$refno'");
...
$sta=$rchkfetch['administration'];
  if($sta=='pending'){
    ...
    mysqli_query($mycon1,"SELECT * FROM productout_tempsave WHERE refno='$refno' INTO OUTFILE '/opt/lampp/htdocs/2021codes/ChargeCart/DelLog/$refno-$da-$ccname.txt' FIELDS TERMINATED BY '|'");
    mysqli_query($mycon1,"DELETE FROM productout_tempsave WHERE refno='$refno'");
    echo "<span class='arial s16 red bold'>Item deleted.</span>";
    echo "<META HTTP-EQUIV='Refresh'CONTENT='1;URL=http://$setip/2021codes/ChargeCart/ccpharmacy_tempsave.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
  }
  else{
    echo "<span class='arial s16 red bold'>Unable to delete item. Item already dispensed.</span>";
    echo "<META HTTP-EQUIV='Refresh'CONTENT='3;URL=http://$setip/2021codes/ChargeCart/ccpharmacy_tempsave.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
  }
}
else{
  //KNOW CREDIT LEFT-----------------------------
...
xmlhttp.open("GET","ccpharmacyresult_tempsave.php?caseno='.$caseno.'&station='.$station.'&toh='.$toh.'&tick='.$tick.'&stk='.$stk.'&at='.$totgross.'&cl='.$cl.'&searchme="+document.searchme.searchme.value,true);
xmlhttp.send();
}
...
$a=0;
$asql=mysqli_query($mycon1,"SELECT refno, productdesc, quantity, trantype FROM productout_tempsave WHERE caseno='$caseno' AND administration='pending'");
...
echo "
        <tr>
          ...
          <form name='delete' method='get' action='ccpharmacy_tempsave.php'>
...
echo "
    ...
    <tr>
      <form name='finalize' method='get' action='finalize.php'>
      <td><div align='center'><input type='submit' style='border: solid 2px black;background-color: #33E8FA;height: 35px;width: 100px;color: #000000;font-weight: bold;font-size: 14px;border-radius: 8px;' name='Finalize' value='Finalize' /></div></td>
      <input type='hidden' name='caseno' value='$caseno' />
      <input type='hidden' name='station' value='$station' />
      <input type='hidden' name='toh' value='$toh' />
      <input type='hidden' name='tick' value='$tick' />
