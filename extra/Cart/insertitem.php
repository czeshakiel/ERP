...
if(isset($_GET['autodistro'])){
  ...
  mysqli_query($conn,"INSERT INTO `productout`(`refno`, `invno`, `caseno`, `productcode`, `productdesc`, `sellingprice`, `quantity`, `adjustment`, `gross`, `trantype`, `phic`, `hmo`, `excess`, `date`, `status`, `terminalname`, `loginuser`, `batchno`, `producttype`, `productsubtype`, `approvalno`, `referenceno`, `administration`, `shift`, `location`, `datearray`, `phic1`) VALUES ('$refno', '$invno', '$caseno', '$code', '$itemname', '$sp', '$qty', '$adjustment', '$gross', '$trantype', '$phic1', '$hmo', '$excess', '$pdate', '$status', 'pending', '$ccname', '$tick', '$lotno', '$unit', '$apprno', '$referenceno', '', 'NEW', '$station', '".date("Y-m-d")."', '$phic2')");
  ...
  if($unit=="LABORATORY"){
    ...
    mysqli_query($conn,"INSERT INTO `labpending` (`refno`, `patientidno`, `caseno`, `patientname`, `itemcode`, `productdesc`, `ptype`, `trantype`, `status`, `resultstatus`, `labtype`, `station`, `dateadded`, `timeadded`, `user`, `viewcount`) VALUES ('$refno', '$patientidno', '$caseno', '$pn', '$code', '$itemname', '$unit', '$trantype', '$status', 'pending', '$lotno', '$station', '".date("Y-m-d")."', '".date("H:i:s")."', '$ccname', '0')");
    //}
    //---------------------------------------------------------------------------------------------
  }
  else if($unit=="XRAY"){
