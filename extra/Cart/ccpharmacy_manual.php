...
$rchksql=mysqli_query($conn,"SELECT trantype,administration FROM productout WHERE refno='$refno'");
$rchkfetch=mysqli_fetch_array($rchksql);
$sta=$rchkfetch['administration'];
$tt=$rchkfetch['trantype'];
if($tt=="PAID"){
  echo "<span class='arial s16 red bold'>Unable to delete item. Item already PAID.</span>";
  echo "<META HTTP-EQUIV='Refresh'CONTENT='3;URL=../Cart/ccpharmacy_manual.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
}
else{
  if($sta=='pending'){
    $da=date("YmdHis");
    ...
    echo "<META HTTP-EQUIV='Refresh'CONTENT='1;URL=../Cart/ccpharmacy_manual.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
  }
  else{
  ...
  $losql=mysqli_query($conn,"SELECT policyno, status FROM admission WHERE caseno='$caseno'");
  ...
  if($cl<$totgross){
    $label=" -->  <span style='color: #FF0000;'>EXCEEDED CREDIT LIMIT $cl > $totgross</span>";
  }
  else{
    $label=" --> $totgross | $cl";
  }
...
$dis=0;
if($patst=="LOCKED"){
  $pensql=mysqli_query($conn,"SELECT `refno`, `productdesc`, `invno`, `datearray`, `approvalno` FROM `productout` WHERE `caseno`='$caseno' AND `trantype`='charge' AND `quantity` > '0' AND `administration`='pending' AND (`productsubtype` LIKE '%PHARMACY/MEDICINE%' OR `productsubtype` LIKE '%PHARMACY/SUPPLIES%' OR `productsubtype` LIKE '%MEDICAL SURGICAL SUPPLIES%')");
  ...
  $dissql=mysqli_query($conn,"SELECT `refno`, `productdesc`, `invno`, `datearray`, `approvalno` FROM `productout` WHERE `caseno`='$caseno' AND `trantype`='charge' AND `quantity` > '0' AND `administration`='dispensed' AND (`productsubtype` LIKE '%PHARMACY/MEDICINE%' OR `productsubtype` LIKE '%PHARMACY/SUPPLIES%' OR `productsubtype` LIKE '%MEDICAL SURGICAL SUPPLIES%')");
...
$lcksql=mysqli_query($conn,"SELECT `status` FROM `admission` WHERE `caseno`='$caseno'");
...
echo "
    <tr>
      <td height='80'><div align='left' style='color: red;font-family: arial;font-size: 20px;font-weight: bold;'>PHARMACY CART IS DISABLED!!! $penlabel $dislabel</div></td>
    </tr>
";
}
else{
  if(isset($_GET['manual'])){$temval=mysqli_real_escape_string($conn,$_GET['searchme']);}else{$temval="";}
echo "
...
";
  ...
  if(isset($_GET['manual'])){
    $at=$totgross;
    $cl=$cl;
    $searchme=mysqli_real_escape_string($conn,$_GET['searchme']);
    ...
    $zsql=mysqli_query($conn,"SELECT membership, hmomembership, status FROM admission WHERE caseno='$caseno'");
    ...
    $a=0;
    $asql=mysqli_query($conn,"SELECT r.unit, r.pnf, r.lotno, r.testcode, r.gtestcode, s.code, s.description, s.generic, SUM(s.quantity) AS soh FROM stocktable s, receiving r WHERE s.code=r.code AND r.unit='PHARMACY/MEDICINE' AND (r.description LIKE '%$searchme%' OR r.generic LIKE '$searchme%') AND s.dept='$stk' GROUP BY s.code");
    ...
    //if($co=="kmsci-34-m-210420084619-15-p"){
    //  $soh=$soh+222;
    //}
    ...
    if($pnf!="PNDF"){
    $bg="bgcolor='#FF0000' title='NON PNDF $pnf $adt'";
    }
    else{
    $bg="title='$adt'";
    }

    /*$ysql=mysqli_query($conn,"SELECT philhealth,nonmed  FROM productsmasterlist WHERE code='$co'");
...
echo "
    ...
    if($soh<1){
      if($pnf!="PNDF"){
        echo "
            <td class='div-container'><div align='center' style='color: #FFFFFF;font-family: Arial;font-weight: bold;'>NON-PNDF</div></td>
        ";
      }
      else{
        echo "
    ...
    echo "
          <td height='35'><div align='left' class='arial s13 black'>&nbsp;".strtoupper($ty)."&nbsp;$ccacce</div></td>
        </tr>
    ";
...
$a=0;
$asql=mysqli_query($conn,"SELECT refno, productdesc, quantity, trantype, status FROM productout WHERE caseno='$caseno' AND quantity > 0 AND batchno='$tick'");
$acount=mysqli_num_rows($asql);
...
if($stat=="PAID"){$disa="class='borderred bggrey white s12' disabled";$stla=" --> Paid";}else{$disa="class='borderred bgred white s12'";$stla="";}
...
echo "
        <tr>
          ...
          <form name='delete' method='get' action='ccpharmacy_manual.php'>
