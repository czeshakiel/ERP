...
$sta=$rchkfetch['administration'];
  if($sta=='pending'){
    ...
    echo "<META HTTP-EQUIV='Refresh'CONTENT='1;URL=http://$setip/2021codes/ChargeCart/ccpharmacy_quot.php?caseno=$caseno&station=$station&toh=$toh&tick=$tick&stk=$stk'>";
//  }
//  else{
...
xmlhttp.open("GET","ccpharmacyresult_quot.php?caseno='.$caseno.'&station='.$station.'&toh='.$toh.'&tick='.$tick.'&stk='.$stk.'&at='.$totgross.'&cl='.$cl.'&searchme="+document.searchme.searchme.value,true);
xmlhttp.send();
}
...
<div align='left'>
  <table border='0' cellpadding='0' cellspacing='0'>
    ...
    <tr>
      <td height='10'><div align='left' class='arial s14 bold'>PHARMACY QUOTATION CART | USER: <span class='blue'>".$_COOKIE["ccname"]." $label</span></div></td>
    </tr>
...
echo "
        <tr>
          ...
          <form name='delete' method='get' action='ccpharmacy_quot.php'>
          <td class='b1' width='50'><div align='center'><button type='submit' name='delete' class='borderred bgred white s12'> X </button</div>
		  <input type='hidden' name='caseno' value='$caseno' />
          <input type='hidden' name='station' value='$station' />
          <input type='hidden' name='toh' value='$toh' />
          <input type='hidden' name='tick' value='$tick' />
