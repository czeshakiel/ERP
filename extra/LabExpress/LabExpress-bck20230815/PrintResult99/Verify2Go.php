...
$testno=mysqli_real_escape_string($conn,$_POST['testno']);
...
";
  ...
  if($bcount==0){
    mysqli_query($conn,"INSERT INTO `verifier` (`refno`, `testno`, `caseno`, `user2`, `name2`, `date2`, `time2`) VALUES ('$refno', '$testno', '$caseno', '$username', '$name', '$pdate', '$ptime')");
  }
  else{
    mysqli_query($conn,"UPDATE `verifier` SET `user2`='$username', `name2`='$name' WHERE `refno`='$refno'");
  }

  if($testno!=""){
    $zsql=mysqli_query($conn,"SELECT `lab29` FROM `hematology` WHERE `testno`='$testno' AND `lab28`='testdone' AND `lab29` NOT LIKE '$refno' GROUP BY `lab29`");
    while($zfetch=mysqli_fetch_array($zsql)){
      mysqli_query($conn,"UPDATE `labpending` SET `verified`='1', `redit`='1' WHERE `refno`='".$zfetch['lab29']."'");
    }
  }
