...
$srefno=mysqli_real_escape_string($conn,$_GET['refno']);
...
$a=0;
$asql=mysqli_query($conn,"SELECT p.`refno`, p.`productcode`, p.`productdesc`, p.`invno`, p.`batchno`, p.`datearray`, l.`test` FROM `productout` p, `labtest` l WHERE p.`caseno`='$caseno' AND p.`refno`=l.`refno` AND l.`test`='$stype' AND (p.`status`='Approved' OR p.`status`='PAID') AND p.`terminalname` NOT LIKE 'Testdone' AND p.`productsubtype`='LABORATORY'");
