...
$conn->query("update homemeds set tam='$am AM', tnn='$nn NN', tpm='$pm PM', tmn='$mn MN', duration='$duration' where code='$code' and batchno='$batchno'");
...
<?php
$sqlx7 = "SELECT productdesc, sum(quantity) as quantity, trantype, refno, batchno  FROM productouthm where caseno ='$caseno' and batchno='$batchno' group by productcode";
...
if($timing==""){echo"<button type='button' class='btn btn-danger btn-sm' data-bs-toggle='modal' data-bs-target='#exampleModaladdtiming' data-bs-dismiss='modal'";?> onclick="search2('<?php echo $tam ?>', '<?php echo $tnn ?>', '<?php echo $tpm ?>', '<?php echo $tmn ?>', '<?php echo $duration ?>', '<?php echo $pcode ?>', ', '<?php echo $batchno ?>'');"; <?php echo">Add Timing..</button>";}else{echo"$timing";}
echo"</td>
...
<script>function search2(val1, val2, val3, val4, val5, val6, val7){
...
document.getElementById("batchno").value=val7;
}</script>
