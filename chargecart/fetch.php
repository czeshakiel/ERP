...
$sqlx4 = "SELECT * FROM productout WHERE caseno='$caseno' AND quantity > 0 AND trantype='charge' AND productsubtype NOT LIKE '%OTHERS%' 
AND producttype NOT LIKE '%READERS FEE%' AND approvalno NOT LIKE '%proferfee%' AND approvalno NOT LIKE '%instrument%' AND producttype NOT LIKE '%PAYMENT OF%'";
...
$sqlccv = "SELECT sum(s.quantity) as qtyd, r.description, s.code, r.generic, r.unit FROM stocktable s, receiving r where 
s.code=r.code and s.dept='$dept' and (r.description like '%$search%' or r.generic like '%$search%') and r.unit='PHARMACY/MEDICINE' group by s.code order by r.code";
...
if($gtestcode!="0"){$com="<span class='badge bg-danger'><i class='icofont-ban'></i></span>"; $btncash = ""; $btncharge="";}
if($qty<=0){$btncash = ""; $btncharge="";}

//if($qty>0){
echo"
<tr>
<td width='60%' style='font-size:11px;'><table><tr><td style='font-size:20px;'><i class='icofont-$log'></i></td><td style='font-size:11px;'><b>$desc</td></tr></table></td>
...
<td style='text-align: center;' width='7%'><input type='number' name='qty' value='1' style='padding: 5px; text-align: center; font-size: 12px;'></td>
<td style='text-align: center;' width='15%'>
<table><tr><td>$btncash</td><td></td>"; 
//if(strpos($caseno, "R-")!==false or strpos($caseno, "R-")!==false or strpos($caseno, "R-")!==false){
echo"<td id='presc'><button type='button' class='btn btn-secondary btn-sm' data-bs-toggle='modal' data-bs-target='#exampleModaladdhm' data-bs-dismiss='modal' "; ?> onclick="search('<?php echo $desc ?> (<?php echo $generic ?>)')" <?php echo"><i class='icofont-plus-circle'></i></button></td>";
//}
