...
$bsql=mysqli_query($conn,"SELECT `label`, `logs` FROM `labresults` WHERE `refno`='$refno'");
...
<table border='0' width='100%' cellpadding='0' cellspacing='0'>
  <tr>
    <td><div align='left' style='padding: 5px 5px;'>
      <table border='0' width='100%' cellpadding='0' cellspacing='0'>
        <tr>
          <td class='b1'><div align='center' style='padding: 5px 5px;font-family: arial;font-weight: bold;font-size: 16px;'>Result Edit Logs</div></td>
        </tr>
        <tr>
...
for($x=1;$x<count($lsp);$x++){

echo "
              <tr>
                <td valign='top'><div align='left' style='font-family: arial;font-size: 13px;padding: 3px 5px 3px 3px;'>$x.</div></td>
                <td><div align='left' style='font-family: arial;font-size: 13px;padding: 3px 3px;'>
";

  $lgdsp=preg_split("/\-/",$lsp[$x]);
  for($y=0;$y<count($lgdsp);$y++){
    if($y==(count($lgdsp)-1)){$rend="";}else{$rend="<br />";}
    echo "- ".$lgdsp[$y].$rend;
  }
