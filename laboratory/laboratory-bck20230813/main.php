...
while($axfetch=mysqli_fetch_array($axsql)){
  ...
  $ansql=mysqli_query($conn,"SELECT COUNT(`caseno`) AS `totnew` FROM `productout` WHERE `shift`='NEW' AND `caseno`='$caseno' AND `productsubtype`='LABORATORY'");
...
";
  ...
  if($azcount>0){
echo"
                        <span class='badge bg-warning'>$azcount Pending</span>
";
  }

  if($avcount>0){
echo"
                        <span class='badge bg-danger'>$avcount request for delete</span>
";
  }
