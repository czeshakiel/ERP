...
<td width="20%"><font size='1'>[mm/dd/yyyy]</font><br><input type="date" name="day1" value="<?php echo $day1 ?>" style="text-align: center; height: 25px; padding: 0;"></td>
<td width="5%" align="center">to</td>
<td width="40%"><font size='1'>[mm/dd/yyyy]</font><br><input type="date" name="day2" value="<?php echo $day2 ?>" style="text-align: center; height: 25px; padding: 0;"></b></td>
...
<?php
$sql4 = "select a.patientidno, a.caseno, p.refno, p.productdesc, p.status, p.loginuser, p.terminalname, a.patientidno, a.room, p.invno, p.datearray, p.trantype, p.productsubtype, p.productcode, p.approvalno, p.administration from productout p, admission a where a.caseno=p.caseno and (p.productsubtype LIKE '%HEARTSTATION%' or p.productsubtype LIKE '%ECG%') $queryx and p.status!='CANCELLED' and p.datearray between '$day1' and '$day2' order by p.datearray desc, p.invno desc";
...
$ofr="";
if($status == "PAID"){
$result44t = $conn->query("select * from collection where refno='$refno'");
while($row44t = $result44t->fetch_assoc()){$ofr = " - ".$row44t['ofr'];}
}

if($terminalname == "pending" && $status == "PAID"){
$refund="<a class='dropdown-item' href='refund.php?refno=$refno' title='For Refund Item' onclick='return refundItem();'><i class='bi bi-arrow-down-left-circle'></i> Refund</a>";}
elseif($terminalname == "refund" && $status == "PAID"){$refund="<a class='dropdown-item' href='undorefund.php?refno=$refno' title='Undo Refund Item' onclick='return undoRefund();'><i class='bi bi-arrow-up-right-circle-fill'></i> Undo Refund</a>";}
else{ $refund="";}
...
<td bgcolor='$col' style='color: $col1; font-size: 11px; text-align: center;'>$status $ofr<br>$terminalname</td>
<td bgcolor='$col' style='color: $col1; font-size: 11px; text-align: center;'>
<a tabindex='0' class='btn btn-outline-primary btn-sm' role='button' title='Person Request' data-bs-container='body' data-toggle='popover' data-bs-placement='top' data-bs-content='$loginuser' style='border-radius: 50%; font-size: 12px;'><i class='icofont-user-alt-4'></i></a>
<a tabindex='0' class='btn btn-outline-danger btn-sm' role='button' title='Reader Doctor' data-bs-container='body' data-toggle='popover' data-bs-placement='top' data-bs-content='$read' style='border-radius: 50%; font-size: 12px;'><i class='icofont-man-in-glasses'></i></a>
</td>
<td bgcolor='$col' style='color: $col1; font-size: 11px; text-align: center;'>
<div class='dropdown'>
...
<a class="dropdown-item" href="?generateresult&caseno=<?php echo $caseno ?>&refno=<?php echo $refno ?>&productsubtype=<?php echo $productsubtype.$datax ?>"><i class='icofont-flask'></i> Generate Result</a>
<button type='button' id='iddecking' data-bs-toggle='modal' data-bs-target='#decking' hidden>My Button</button>
<?php } echo"
<a class='dropdown-item' href='http://$ip/cgi-bin/printallsup.cgi?caseno=$caseno&nursename=$user&view=$dept' target='_blank'><i class='icofont-printer'></i> Print Slip</a>
$refund
<a class='dropdown-item' href='?main&caseno=$caseno&refno=$refno&code=$productcode&delete' onclick='confirmx();'><i class='icofont-trash'></i> Delete</a>
</ul>
</div>
</td>
</tr>
