...
.btnstyle .print {background-color: #FFC300;color: #000000;}
.btnstyle .print:hover {opacity: 0.8;}
...
xmlhttp.open("GET","finallistsr.php?view=fl&user='.$user.'&userunique='.$userunique.'&show='.$show.'&page='.$page.'&searchme="+document.searchme.searchme.value,true);
xmlhttp.send();
}
';
?>
    ...
    <tr>
      <td><div align='left' class='tabstyle'><table border='0' cellapdding='0' cellspacing='0'>
        <tr>
          ...
          <td width='2' bgcolor='#ece0cf' style='border-bottom: 2px solid #ece0cf;'></td>
          <td class='b2'><a href='../philhealth/?fl&show=$show' style='text-decoration: none;'><div align='center' class='tabselect'>Transmittal List</div></a></td>
          <td width='2' bgcolor='#ece0cf' style='border-bottom: 2px solid #ece0cf;'></td>
        ...
        <tr>
          <td width='50%'><div align='left'><table border='0' cellpadding='0' cellspacing='0'>
            <tr>
              ...
              <td width='5'></td>
              <form id='NextPage' name='NextPage' method='post' action='../philhealth/?fl&page=0'>
          ...
          <form name='searchme' onload='showResult();' method='post' action='../philhealth/?fl&show=$show&page=".($page)."'>
          <td width='50%'><div align='right' style='font-family: arial; font-weight: bold; font-size: 12px; color: blue;'>SEARCH: <input name='searchme' type='text' autocomplete='off' onKeyUp='showResult();' style='height: 25px;width: 250px;font-family: courier new;color: red;border-top: none;border-left: none;border-right: none;border-bottom: 2px solid black;padding-left: 5px;padding-right: 5px;' placeholder='Transmittal Number'></div></td>
          </form>
        </tr>
      </table></td>
    </tr>
...
$a=$page;
$asql=mysqli_query($conn,"SELECT `claimnumber`, `datetransmitted` FROM `translist` WHERE `status`='Finalized' GROUP BY `claimnumber` ORDER BY `claimnumber` DESC LIMIT $page,$show");
...
$pf=0;
$tt=0;
$bsql=mysqli_query($conn,"SELECT `roomandboard`, `labothers`, `meds`, `or`, `pf` FROM `translist` WHERE `claimnumber`='$claimnumber'");
...
$csql=mysqli_query($conn,"SELECT * FROM `translist` WHERE `claimnumber`='$claimnumber'");
...
echo "
                  <tr>
                    ...
                    <td class='b1 l1 r2' valign='middle'><div align='center' class='arial s14 black btnstyle'><table border='0' cellpadding='0' cellspacing='0'>
                      <tr>
                      <td><button type='button' class='btn import' title='Edit'"; ?> onclick="<?php echo "window.open('FinalizeEdit.php?claimnumber=$claimnumber&uname=$user', '_blank', 'toolbar=yes,scrollbars=yes,resizable=yes,top=380,left=720,width=450,height=250');";?>" <?php echo "><i class='icofont-edit'></i></button></td>
                      <td width='2'></td>
                      <td><a href='../philhealth/?dl&show=$show&claimnumber=$claimnumber'><button type='button' class='btn view' title='View Details'><i class='icofont-eye-alt'></i></button></a></td>
                      <td width='2'></td>
                      <td><a href='../extra/TransmittalList/?claimnumber=$claimnumber' target='_blank'><button type='button' class='btn print' title='Print Transmittal List'><i class='icofont-print'></i></i></button></a></td>
                      </tr>
                    </table></div></td>
                  </tr>
";
...
$pagesql=mysqli_query($conn,"SELECT `claimnumber`, `datetransmitted` FROM `translist` WHERE `status`='Finalized' GROUP BY `claimnumber`");
...
echo "
                            <tr>
                              ...
                              <td width='2'></td>
                              <form name='ShortPage' method='post' action='../philhealth/?fl&show=$show'>
                              <td><div align='center'>
                              ...
                              <td width='2'></td>
                              <form id='NextPage' name='NextPage' method='post' action='../philhealth/?fl&show=$show&page=".($nxtpage)."'>
                              <td>
...
echo "
                            <tr>
                              <form id='NextPage' name='NextPage' method='post' action='../philhealth/?fl&show=$show&page=".($prevpage)."'>
                              <td>
