...
$ot=0;
$otsql=mysqli_query($conn,"SELECT * FROM `productout` where `caseno`='$caseno' AND `quantity` > 0  AND `producttype` NOT LIKE '%READERS FEE%' AND `administration` NOT LIKE '%pending%' AND (`productsubtype`='ROOM ACCOMODATION' OR `productsubtype`='PROFESSIONAL FEE' OR `productsubtype`='NURSING CHARGES' OR `productsubtype`='NURSING SERVICE FEE' OR `productsubtype`='OR/DR/ER FEE' OR `productsubtype`='MISCELLANEOUS' OR `productsubtype`='OTHER FEES' OR `productsubtype`='CONSULTATION FEE' OR `productsubtype`='ADMISSION FEE' OR `productsubtype`='AMBULANCE INCOME' OR `productsubtype`='ER FEE' OR `productsubtype`='NURSE ON CALL' OR `productsubtype` LIKE '%OXYGEN%' OR `productsubtype` LIKE '%MEDICAL EQUIPMENT%' OR `productsubtype` LIKE 'OR-CHARGES' OR `productsubtype` LIKE 'LINENS' OR `productsubtype` LIKE 'GENERAL SUPPLIES' OR `productsubtype` LIKE '%PULMONARY%' OR `productsubtype` LIKE '%RT REFERRAL%' OR `productsubtype` LIKE '%OPERATING ROOM FEE%') AND (`status`='Approved' OR `status`='PAID' OR `status`='BALANCE' OR `status`='requested' OR `status`='REFUND') GROUP BY `refno` ORDER BY `datearray` DESC");
