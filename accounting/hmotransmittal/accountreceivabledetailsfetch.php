...
$queryDischargedDate = $conn->query("SELECT * FROM dischargedtable WHERE caseno ='$caseno'");

if(mysqli_num_rows($result22j) > 0){
$fth = $queryDischargedDate->fetch_assoc(); 
$dtDischarged = $fth['datearray'];
while($rres = $result22j->fetch_assoc()){
$id = $rres['autono'];
$transdate = $rres['transdate'];
$datedischarged = $dtDischarged;
...
<td style='font-size: 12px; background: $col;' ><font color='$fcolor'><i class='icofont-id'></i> Caseno:</font> <a href='../nsstation/?detail&caseno=$caseno' target='_blank'>$caseno</a><br><font color='$fcolor'><i class='icofont-user-alt-3'></i> Name:</font> <b>$pname</b><br><font color='$fcolor'><i class='icofont-calendar'></i> Date Transmit:</font> <b>$transdate</b><br><font color='$fcolor'><i class='icofont-calendar'></i> Date Admitted:</font> <b>$dateadmit</b><br><font color='$fcolor'><i class='icofont-calendar'></i> Date Discharged:</font> <b>$datedischarged</b></td>
