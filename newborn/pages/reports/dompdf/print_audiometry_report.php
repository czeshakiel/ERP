...
$datef = $_POST['audDateFrom'];
$datet = $_POST['audDateTo'];
...
$heading = $res['heading'];
$address = $res['address'];
$queryAudiometry = $pdo->query("SELECT refno, referenceno, caseno, productcode, productdesc, datearray FROM `productout` WHERE productcode = '$code' AND terminalname='Testdone' AND datearray BETWEEN '$datef' AND '$datet' ORDER BY datearray ASC");
    ...
    <html>
        ...
        <body>
            ...
            <table width='100%' border='0' cellpadding='0' cellspacing='0'>
                ...
                <tr>
                    <td class='arial14C'><b>AUDIOMETRY READER PF REPORT</b></td>
                </tr>
                <tr>
            ...
            <table width='100%' border='1' cellpadding='0' cellspacing='0' class='pddtp20'>
                <thead>
                    <tr>
                        ...
                        <th class='arial12C' style='width: 15%'>Caseno</th>
                        <th class='arial12C' style='width: 35%'>Patient Name</th>
                        <th class='arial12C' style='width: 35%'>Reader</th>
                        <th class='arial12C' style='width: 10%'>Date</th>
                    </tr>
                </thead>
                <tbody>";
                    $cnt = 0;
                    if($queryAudiometry->rowCount() > 0){
                        while($rslt = $queryAudiometry->fetch(PDO::FETCH_ASSOC)){
                            $cnt++; 
                            $caseno = $rslt['caseno'];
                            $prodcode = $rslt['productcode'];
                            $proddesc = $rslt['productdesc'];
                            $refno = $rslt['refno'];
                            $reader = $rslt['referenceno'];
                          
                            if(is_string($reader)){
                                $readername = $reader; }
                            if(is_numeric($reader)){
                                $queryReader = $pdo->query("SELECT * FROM nsauthdoctors WHERE empid = '$reader'");
                                $ftr = $queryReader->fetch(PDO::FETCH_ASSOC);
                                $readername = $ftr['name'];
                            }
                            $dtarray = $rslt['datearray'];
                ...
                $html .="
                    <tr>
                        ...
                        <td class='arial12C'>$readername</td>
                        <td class='arial12C'>$dtarray</td>
                    </tr>";
                        }
                    }else{
                ...
                <tr>
                    <td class='arial12C'><span>Audiometrecian</span></td>
                    <td></td>
