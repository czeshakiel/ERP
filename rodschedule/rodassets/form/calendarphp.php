...
{
    parent::__construct('L', 'mm', array(210, 330));
}
// Page header
function Header()
{   
    ...
    $this->cell(70,30, $this->Image('prop/mmshi.png', 40, 10, 30),  0,0,'C');
    $this->Cell(170,10,'KIDAPAWAN MEDICAL SPECIALIST CENTER, INC.', 0,0,'C');
    $this->Ln();
    $this->SetFont('Helvetica','B',8);
    $this->Cell(310,5,'BRGY. SUDAPIN, KIDAPAWAN CITY',0,0,'C');
    $this->Ln();
    $this->Cell(310,5,'TEL. NO.: (064)577-4553',0,0,'C');
    $this->Ln(10);
    $this->SetFont('Helvetica','B',12);
    $this->SetTextColor(255,103,0);
    $this->Cell(310,10,'ROD SCHEDULE FOR THE MONTH OF '.$monthName.' '.$y , 0,0,'C');
    $this->Ln(20);
...
function CalendarTable() {
        $condb = new PDO('mysql::host=localhost;dbname=kmsci','root','b0ykup4l');
        ...
        $this->SetFont('Arial', 'B', 8.5); $this->SetTextColor(0, 0, 102); $lbsWidth = 30; $lbsHeight = 10; $dLabelX = 20; $dLabelY = 45;
        ...
        for ($day = 1 - $this->first_day; $day <= $this->num_days; $day++) {
            ...
            if ($day > 0) {
                ...
                foreach ($stations as $station_id => $lastnames) {
                    ...
                    $this->SetXY($xPos, $yPos + $cellMargin + 5* ($station_id - 1)); // set Y position based on station_id
                    $this->MultiCell($cellWidth, $cellMargin + 0.5, $names_string, 0, 'C');
                }
            ...
            if ($day == $this->num_days && ($day + $this->first_day) % 7 != 0) {
                // Add empty cells to fill the space
                $remaining_cells = 7 - ($day + $this->first_day) % 7;
        ...
        for($i = 0; $i < $num_rows; $i++){
            $this->MultiCell($cellWidth, $cellHeight, implode("\n", $stations), 1, 'C');
