...
$sumifru1beg=0;$eastwest1beg=0;$generali1beg=0;$medcard1beg=0;$prudent1beg=0;$clinic1beg=0;$mplus1beg=0;$personal1beg=0;$employee1beg=0;$office1beg=0;$phc1beg=0;$catamco1beg=0;$tejada1beg=0;$mcp1beg=0;$makmpc1beg=0;$mediatrix1beg=0;$cotelco1beg=0;$sssbeg=0;$sss1beg=0;$edc1beg=0;
$totalamountbeg=0;
$credittotalbeg=0;
...
if($department=="O-"){
  $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE c.datearray >= '$checkdate' AND c.datearray < '$rundate' AND a.status='discharged' AND (a.caseno LIKE '%$department%' OR a.caseno LIKE '%AR-%') AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
}else{
...
$clinic1=0;$personal1=0;$employee1=0;$office1=0;$phc1=0;$catamco1=0;$tejada1=0;$mcp1=0;$makmpc1=0;$mediatrix1=0;$edc1=0;
$totalamount=0;
$credittotal=0;
  if($department=="O-"){
    $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE c.datearray = '$rundate' AND a.status='discharged' AND (a.caseno LIKE '%$department%' OR a.caseno LIKE '%AR-%') AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
  }else{
    $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE ((c.datearray = '$startdate' AND c.paymentTime >= '23:00:00') OR (c.datearray = '$enddate' AND c.paymentTime < '23:00:00')) AND a.status='discharged' AND a.caseno LIKE '%$department%' AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'AR DSWD' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
  }
    if(mysqli_num_rows($sqlPatient)>0){
      while($patient=mysqli_fetch_array($sqlPatient)){
        $caseno =$patient['caseno'];
