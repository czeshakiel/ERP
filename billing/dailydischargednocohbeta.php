...
$cohbeg=0;$depbeg=0;$discbeg=0;$arphicbeg=0;$arpfphicbeg=0;$albeg=0;$avegabeg=0;$lovejbeg=0;$chealthbeg=0;$caritasbeg=0;$cocobeg=0;$flexibeg=0;$intelbeg=0;$maxibeg=0;$mclbeg=0;$mcardbeg=0;$pcarebeg=0;$vcarebeg=0;$insulifebeg=0;
...
$coh1beg=0;$dep1beg=0;$disc1beg=0;$arphic1beg=0;$arpfphic1beg=0;$al1beg=0;$avega1beg=0;$lovej1beg=0;$chealth1beg=0;$caritas1beg=0;$coco1beg=0;$flexi1beg=0;$intel1beg=0;$maxi1beg=0;$mcl1beg=0;$mcard1beg=0;$pcare1beg=0;$employee1beg=0;$pgicare1beg=0;$caoagdan1beg=0;$pinol1beg=0;
...
$sumifru1beg=0;$eastwest1beg=0;$generali1beg=0;$medcard1beg=0;$prudent1beg=0;$clinic1beg=0;$mplus1beg=0;$personal1beg=0;$employee1beg=0;$office1beg=0;$phc1beg=0;$catamco1beg=0;$tejada1beg=0;$mcp1beg=0;$makmpc1beg=0;$mediatrix1beg=0;$cotelco1beg=0;$sssbeg=0;$sss1beg=0;$disc1begsen=0;$disc1begnorm=0;$edc1beg=0;
$totalamountbeg=0;
$credittotalbeg=0;
...
$finalizebeg=mysqli_query($conn,"SELECT pp.lastname,pp.firstname,pp.middlename,pp.suffix,a.caseno,a.dateadmit,a.employerno,a.hmo,a.addemployer FROM admission a INNER JOIN patientprofile pp ON pp.patientidno=a.patientidno INNER JOIN arv_tbl_hmofinalize atm ON atm.caseno=a.caseno WHERE atm.datearray >= '$checkdate' AND atm.datearray < '$rundate' AND atm.status <> ''");
$finalizenow=mysqli_query($conn,"SELECT pp.lastname,pp.firstname,pp.middlename,pp.suffix,a.caseno,a.dateadmit,a.employerno,a.hmo,a.addemployer FROM admission a INNER JOIN patientprofile pp ON pp.patientidno=a.patientidno INNER JOIN arv_tbl_hmofinalize atm ON atm.caseno=a.caseno WHERE atm.datearray='$rundate' AND atm.status <> ''");
if($department=="O-"){
  $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE c.datearray >= '$checkdate' AND c.datearray < '$rundate' AND a.status='discharged' AND (a.caseno LIKE '%$department%') AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
}else{
  $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE CONCAT(c.datearray,' ',c.paymentTime) >= '$datefrom' AND CONCAT(c.datearray,' ',c.paymentTime) < '$datestart' AND a.status='discharged' AND a.caseno LIKE '%$department%' AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
}
...
if($cohbeg==0){
        ...
        $check_disc=mysqli_query($conn,"SELECT * FROM admission WHERE caseno='$caseno' AND senior='Y'");
        if(mysqli_num_rows($check_disc)>0){
          $disc1begsen += $discbeg;
        }else{
          $disc1begnorm += $discbeg;
        }

        $sql=mysqli_query($conn,"SELECT SUM(amount) as amount FROM acctgenledge WHERE caseno='$caseno' AND acctitle='DISCOUNT' GROUP BY acctitle");
        if(mysqli_num_rows($sql)>0){
            $result=mysqli_fetch_array($sql);
            $disc1beg += $result['amount'];
            $disc1begnorm += $result['amount'];
        }
		...
		$sql=mysqli_query($conn,"SELECT SUM(po.hmo) AS amount FROM productout po LEFT JOIN admission a ON a.caseno=po.caseno WHERE a.caseno='$caseno' AND po.trantype='charge' AND po.quantity > 0 AND a.hmo='WITH LOVE JAN FOUNDATION' AND po.productsubtype NOT LIKE '%PROFESSIONAL FEE%'");
        $amount=mysqli_fetch_array($sql);
        $lovejbeg=$amount['amount'];
        $lovej1beg +=$lovejbeg;
        ...
        $sql=mysqli_query($conn,"SELECT SUM(po.hmo) AS amount FROM productout po LEFT JOIN admission a ON a.caseno=po.caseno WHERE a.caseno='$caseno' AND po.trantype='charge' AND po.quantity > 0 AND a.hmo='VALU CARE' AND po.productsubtype NOT LIKE '%PROFESSIONAL FEE%'");
        $amount=mysqli_fetch_array($sql);
	...
	$amount=mysqli_fetch_array($sql);
        ...
        $sql=mysqli_query($conn,"SELECT SUM(amount) AS amount FROM collection WHERE accttitle='AR EDC' AND acctno='$caseno' AND refno NOT LIKE '%LP%'");
        $amount=mysqli_fetch_array($sql);
        $pcso=$amount['amount'];
        $edc1beg +=$pcso;

        $sql=mysqli_query($conn,"SELECT SUM(po.hmo) AS amount FROM productout po INNER JOIN admission a ON a.caseno=po.caseno WHERE a.hmo='EDC' AND a.caseno='$caseno' AND po.productsubtype NOT LIKE '%PROFESSIONAL FEE%'");
        $amount=mysqli_fetch_array($sql);
        $pcso=$amount['amount'];
        $edc1beg +=$pcso;
...
$coh1=0;$dep1=0;$disc1=0;$arphic1=0;$arpfphic1=0;$al1=0;$avega1=0;$lovej1=0;$chealth1=0;$caritas1=0;$coco1=0;$flexi1=0;$intel1=0;$maxi1=0;$mcl1=0;$mcard1=0;$pcare1=0;$vcare1=0;
$dswd1=0;$lgumak1=0;$lgukid1=0;$pcso1=0;$pepsi1=0;$province1=0;$trade1=0;$tradepf1=0;$pcross1=0;$maip1=0;$insu1=0;$fortune1=0;$stanfilco1=0;$doctor1=0;$sumifru1=0;$eastwest1=0;$generali1=0;$prudent1=0;$pgicare1=0;$insulife1=0;$cotelco1=0;$santos1=0;$caoagdan1=0;$pinol1=0;$sss1=0;$edc1=0;
$clinic1=0;$personal1=0;$employee1=0;$office1=0;$phc1=0;$catamco1=0;$tejada1=0;$mcp1=0;$makmpc1=0;$mediatrix1=0;$discsen1=0;$discnorm1=0;
$totalamount=0;
$credittotal=0;
  if($department=="O-"){
    $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE c.datearray = '$rundate' AND a.status='discharged' AND (a.caseno LIKE '%$department%') AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
  }else{
    $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE ((c.datearray = '$startdate' AND c.paymentTime >= '23:00:00') OR (c.datearray = '$enddate' AND c.paymentTime < '23:00:00')) AND a.status='discharged' AND a.caseno LIKE '%$department%' AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
  }
    if(mysqli_num_rows($sqlPatient)>0){
      while($patient=mysqli_fetch_array($sqlPatient)){
        $caseno =$patient['caseno'];
...
if(mysqli_num_rows($finalizenow)>0 && $department=="O-"){
      while($patient=mysqli_fetch_array($finalizenow)){
        $caseno =$patient['caseno'];
        // $p=explode(' ',$patient['patientname']);
        // $patientname =$p[0].", ".$p[1];        
        $patientname=$patient['patientname'];
...
$credittotal=$coh1+$dep1+$disc1+$arphic1+$arpfphic1+$al1+$avega1+$lovej1+$chealth1+$caritas1+$coco1+$flexi1+$intel1+$maxi1+$mcl1+$mcard1+$pcare1+$vcare1+$pcross1+$dswd1+$lgumak1+$lgukid1+$pcso1+$pepsi1+$province1+$trade1+$tradepf1+$maip1+$insu1+$fortune1+$stanfilco1+$doctor1+$sumifru1+$eastwest1+$generali1+$medcard1+$prudent1+$clinic1+$mplus1+$personal1+$employee1+$office1+$phc1+$catamco1+$tejada1+$mcp1+$makmpc1+$mediatrix1+$pgicare1+$insulife1+$cotelco1+$santos1+$caoagdan1+$pinol1+$balfour1+$sss1+$edc1;
...
$totalamount=0;
$totalcreditbeg=0;
$finalizebeg1=mysqli_query($conn,"SELECT pp.lastname,pp.firstname,pp.middlename,pp.suffix,a.caseno,a.dateadmit,a.employerno,a.hmo,a.addemployer FROM admission a INNER JOIN patientprofile pp ON pp.patientidno=a.patientidno INNER JOIN arv_tbl_hmofinalize atm ON atm.caseno=a.caseno WHERE atm.datearray >= '$checkdate' AND atm.datearray < '$rundate' AND atm.status <> ''");
$finalizenow1=mysqli_query($conn,"SELECT pp.lastname,pp.firstname,pp.middlename,pp.suffix,a.caseno,a.dateadmit,a.employerno,a.hmo,a.addemployer FROM admission a INNER JOIN patientprofile pp ON pp.patientidno=a.patientidno INNER JOIN arv_tbl_hmofinalize atm ON atm.caseno=a.caseno WHERE atm.datearray='$rundate' AND atm.status <> ''");
if($department=="O-"){
  $sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE c.datearray >= '$checkdate' AND c.datearray < '$rundate' AND a.status='discharged' AND (a.caseno LIKE '%$department%') AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
}else{
$sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE CONCAT(c.datearray,' ',c.paymentTime) >= '$datefrom' AND CONCAT(c.datearray,' ',c.paymentTime) < '$datestart' AND a.status='discharged' AND a.caseno LIKE '%$department%' AND (((c.accttitle = 'AR TRADE' OR c.accttitle LIKE '%AR TRADE%' OR c.accttitle = 'NO EXCESS' OR c.accttitle LIKE '%AR EMPLOYEE%' OR c.accttitle LIKE '%AR DOCTOR%') AND c.type='pending') OR (c.accttitle='PROFESSIONAL FEE' AND c.type='cash-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
...
if(mysqli_num_rows($finalizebeg1)>0  && $department=="O-"){
      while($patient=mysqli_fetch_array($finalizebeg1)){
        $caseno =$patient['caseno'];
        // $p=explode(' ',$patient['patientname']);
        // $patientname =$p[0].", ".$p[1];        
        $patientname=$patient['patientname'];
...
if(mysqli_num_rows($finalizenow1)>0 && $department=="O-"){
      while($patient=mysqli_fetch_array($finalizenow1)){
        $caseno =$patient['caseno'];
        // $p=explode(' ',$patient['patientname']);
        // $patientname =$p[0].", ".$p[1];
...
<body>
  <div class="page">
    ...
    <table width="100%" border="0">
      <tr>
        ...
        <td>
          ...
          <b style="font-family:'Times New Roman'; font-size:12px;"><?=date('F d, Y',strtotime($startdate));?> 11:00 PM to <?=date('F d, Y',strtotime($enddate));?> 11:00 PM</b>
        </td>
      </tr>
    </table>
    <br>
    <table width="50%" border="0" style="font-size:12px;">
        ...
        <tr>
            <td>DISCOUNT SENIOR</td>
            <td align="right"><?=number_format($disc1begsen,2);?></td>
            <td align="right"><?=number_format($discsen1,2);?></td>
            <td align="right"><?=number_format($disc1begsen+$discsen1,2);?></td>
        </tr>
        <tr>
            <td>DISCOUNT REGULAR</td>
            <td align="right"><?=number_format($disc1begnorm,2);?></td>
            <td align="right"><?=number_format($discnorm1,2);?></td>
            <td align="right"><?=number_format($disc1begnorm+$discnorm1,2);?></td>
        </tr>
        <tr>
...
<?php
	...
	if($balfour1beg > 0 || $balfour1 > 0){
        ...
        if($lovej1beg > 0 || $lovej1 > 0){
        ?>
        <tr>
            <td>ACCOUNTS RECEIVABLE - LOVE JAN FOUNDATION</td>
            <td align="right"><?=number_format($lovej1beg,2);?></td>
            <td align="right"><?=number_format($lovej1,2);?></td>
            <td align="right"><?=number_format($lovej1beg+$lovej1,2);?></td>
        </tr>
        ...
        ?>
        <tr>
            <td>ACCOUNTS RECEIVABLE - VALU CARE</td>
        ...
        ?>
        <tr>
            <td>ACCOUNTS RECEIVABLE - EDC</td>
            <td align="right"><?=number_format($edc1beg,2);?></td>
            <td align="right"><?=number_format($edc1,2);?></td>
            <td align="right"><?=number_format($edc1beg+$edc1,2);?></td>
        </tr>
        <?php
        }
        if($mcl1beg > 0 || $mcl1 > 0){
