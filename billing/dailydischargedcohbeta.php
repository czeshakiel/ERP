...
$datestart=date('Y-m-d',strtotime($startdate))." 22:59:59";
...
$cohbeg=0;$depbeg=0;$discbeg=0;$arphicbeg=0;$arpfphicbeg=0;$albeg=0;$avegabeg=0;$chealthbeg=0;$caritasbeg=0;$cocobeg=0;$flexibeg=0;$intelbeg=0;$maxibeg=0;$mclbeg=0;$mcardbeg=0;$pcarebeg=0;$vcarebeg=0;$cotelcobeg=0;$santosbeg=0;$insulifebeg=0;$caoagdanbeg=0;$pinolbeg=0;$medoarebeg=0;
$dswdbeg=0;$lgumakbeg=0;$lgukidbeg=0;$pcsobeg=0;$pepsibeg=0;$provincebeg=0;$tradebeg=0;$tradepfbeg=0;$pcrossbeg=0;$maipbeg=0;$insubeg=0;$fortunebeg=0;$stanfilcobeg=0;$doctorbeg=0;$sumifrubeg=0;$easwestbeg=0;
...
$coh1beg=0;$dep1beg=0;$disc1beg=0;$arphic1beg=0;$arpfphic1beg=0;$al1beg=0;$avega1beg=0;$chealth1beg=0;$caritas1beg=0;$coco1beg=0;$flexi1beg=0;$intel1beg=0;$maxi1beg=0;$mcl1beg=0;$mcard1beg=0;$pcare1beg=0;$balfour1beg=0;
$vcare1beg=0;$dswd1beg=0;$lgumak1beg=0;$lgukid1beg=0;$pcso1beg=0;$pcso2beg=0;$pepsi1beg=0;$province1beg=0;$trade1beg=0;$tradepf1beg=0;$pcross1beg=0;$maip1beg=0;$insu1beg=0;$forutne1beg=0;$stanfilco1beg=0;$doctor1beg=0;$cotelco1beg=0;$santos1beg=0;$caoagdan1beg=0;$pinol1beg=0;
$sumifru1beg=0;$eastwest1beg=0;$generali1beg=0;$medcard1beg=0;$prudent1beg=0;$clinic1beg=0;$mplus1beg=0;$personal1beg=0;$employee1beg=0;$office1beg=0;$phc1beg=0;$catamco1beg=0;$tejada1beg=0;$mcp1beg=0;$makmpc1beg=0;$insulife1beg=0;$mediatrix1beg=0;$pgicare1beg=0;$medocare1beg=0;$edc1beg=0;
$disc1begsen=0;$disc1begnorm=0;
$totalamountbeg=0;
$credittotalbeg=0;
$sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE CONCAT(c.datearray,' ',c.paymentTime) >= '$datefrom' AND CONCAT(c.datearray,' ',c.paymentTime) <= '$datestart' AND (a.status='discharged' OR a.ward='discharged') AND a.caseno LIKE '%$department%' AND ((c.accttitle='CASHONHAND' AND (c.type='cash-Visa' OR c.type='card-Visa')) OR (c.accttitle='PROFESSIONAL FEE' AND c.type='card-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
    if(mysqli_num_rows($sqlPatient)>0){
      while($patient=mysqli_fetch_array($sqlPatient)){
        ...
        // $sql=mysqli_query($conn,"SELECT SUM(amount) AS amount FROM collection WHERE ((accttitle='CASHONHAND' AND `type` = 'cash-Visa') OR (accttitle ='PROFESSIONAL FEE' AND `type`='card-Visa')) AND acctno='$caseno'");
        ...
        $sql=mysqli_query($conn,"SELECT SUM(po.hmo) AS amount FROM productout po LEFT JOIN admission a ON a.caseno=po.caseno WHERE a.caseno='$caseno' AND po.trantype='charge' AND po.quantity > 0 AND a.hmo='MEDOCARE' AND po.productsubtype NOT LIKE '%PROFESSIONAL FEE%'");
        $amount=mysqli_fetch_array($sql);
...
$sql=mysqli_query($conn,"SELECT SUM(po.hmo) AS amount FROM productout po LEFT JOIN admission a ON a.caseno=po.caseno WHERE a.caseno='$caseno' AND po.trantype='charge' AND po.quantity > 0 AND a.hmo='INSULAR LIFE' AND po.productsubtype NOT LIKE '%PROFESSIONAL FEE%'");
        ...
        $sql=mysqli_query($conn,"SELECT SUM(amount) AS amount FROM collection WHERE acctno='$caseno' AND accttitle='AR TRADE' AND refno NOT LIKE '%LP%'");
        $amount=mysqli_fetch_array($sql);
  ...
  if($rundate==$checkdate){
    $coh1beg=0;$dep1beg=0;$disc1beg=0;$arphic1beg=0;$arpfphic1beg=0;$al1beg=0;$avega1beg=0;$chealth1beg=0;$caritas1beg=0;$coco1beg=0;$flexi1beg=0;$intel1beg=0;$maxi1beg=0;$mcl1beg=0;$mcard1beg=0;$pcare1beg=0;$pgicare1beg=0;$insulife1beg=0;
...
$dswd=0;$lgumak=0;$lgukid=0;$pcso=0;$pepsi=0;$province=0;$trade=0;$tradepf=0;$pcross=0;$maip=0;$insu=0;$fortune=0;$stanfilco=0;$doctor=0;$sumifru=0;$eastwest=0;$generali=0;$prudent=0;$clinic=0;$personal=0;$santos=0;$caoagdan=0;$pinol=0;$insulife=0;
$employee=0;$balfour1=0;$edc1=0;
...
$dswd1=0;$lgumak1=0;$lgukid1=0;$pcso1=0;$pcso2=0;$pepsi1=0;$province1=0;$trade1=0;$tradepf1=0;$pcross1=0;$maip1=0;$insu1=0;$fortune1=0;$stanfilco1=0;$doctor1=0;$sumifru1=0;$eastwest1=0;$generali1=0;$prudent1=0;$pgicare1=0;$cotelco1=0;$santos1=0;$caoagdan1=0;$pinol1=0;$cotelco2=0;
$clinic1=0;$personal1=0;$employee1=0;$office1=0;$phc1=0;$catamco1=0;$tejada1=0;$mcp1=0;$makmpc1=0;$mediatrix1=0;$insulife1=0;$discsen1=0;$discnorm1=0;
$totalamount=0;
$credittotal=0;
$sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE ((c.datearray = '$startdate' AND c.paymentTime >= '23:00:00') OR (c.datearray = '$enddate' AND c.paymentTime <= '23:00:00')) AND (a.status='discharged' OR a.ward='discharged') AND a.caseno LIKE '%$department%' AND ((c.accttitle='CASHONHAND' AND (c.type='cash-Visa' OR c.type='card-Visa')) OR (c.accttitle='PROFESSIONAL FEE' AND c.type='card-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
    if(mysqli_num_rows($sqlPatient)>0){
      while($patient=mysqli_fetch_array($sqlPatient)){
        $caseno =$patient['caseno'];
...
$sqlPatient=mysqli_query($conn,"SELECT a.caseno,CONCAT(pp.lastname,', ',pp.firstname,' ',pp.middlename) as patientname,SUM(c.amount) as amount FROM admission a LEFT JOIN collection c ON c.acctno=a.caseno LEFT JOIN patientprofile pp ON pp.patientidno=a.patientidno WHERE CONCAT(c.datearray,' ',c.paymentTime) >= '$datefrom' AND CONCAT(c.datearray,' ',c.paymentTime) <= '$datestart' AND (a.status='discharged' OR a.ward='discharged') AND a.caseno LIKE '%$department%' AND ((c.accttitle='CASHONHAND' AND (c.type='cash-Visa' OR c.type='card-Visa')) OR (c.accttitle='PROFESSIONAL FEE' AND c.type='card-Visa')) AND c.refno NOT LIKE '%LP%' GROUP BY a.caseno ORDER BY pp.lastname ASC");
    if(mysqli_num_rows($sqlPatient)>0){
      while($patient=mysqli_fetch_array($sqlPatient)){
        ...
        //  $sql=mysqli_query($conn,"SELECT SUM(amount) AS amount FROM collection WHERE acctno='$caseno' AND accttitle LIKE '%REFUND%' AND `type`='pending'");
        // $amount=mysqli_fetch_array($sql);
        ...
        $sql=mysqli_query($conn,"SELECT SUM(amount) AS amount FROM collection WHERE acctno='$caseno' AND (accttitle LIKE '%AP PATIENT DEPOSIT%' OR accttitle LIKE '%FOR REFUND%')");
        $amount=mysqli_fetch_array($sql);
  ...
  $totalcreditbeg=$meds1beg+$lab1beg+$xray1beg+$utz1beg+$ecg1beg+$eeg1beg+$ct1beg+$echo1beg+$respi1beg+$room1beg+$sup1beg+$rt1beg+$erfee1beg+$admit1beg+$nsfee1beg+$ordr1beg+$oxygen1beg+$cert1beg+$diet1beg+$pt1beg+$ambu1beg+$appfother1beg+$appftrade1beg+$appfphic1beg+$appfdep1beg+$ordrsupplies1beg+$refund1beg;
}
  ...
  <div class="page">
    ...
    <table width="100%" border="0">
      <tr>
        ...
        <td>
          <b style="font-family:'Times New Roman'; font-size:12px;">DISCHARGED REPORT (COH) - SUMMARY</b><br />
...
<?php
	...
	<?php
        ...
        if($edc1beg+$pcso2beg > 0 || $edc1+$pcso2 > 0){
        ?>
        <tr>
            <td>ACCOUNTS RECEIVABLE - EDC</td>
            <td align="right"><?=number_format($edc1beg,2);?></td>
            <td align="right"><?=number_format($edc1+$pcso2,2);?></td>
